// Generated from catalog/listings.json by npm run listings:generate. Do not edit directly.
export const CITY_PRESETS = [
  {
    "name": "Plano",
    "label": "Plano, TX",
    "lat": 33.0198,
    "lng": -96.6989
  },
  {
    "name": "Dallas",
    "label": "Dallas, TX",
    "lat": 32.7767,
    "lng": -96.797
  },
  {
    "name": "Fort Worth",
    "label": "Fort Worth, TX",
    "lat": 32.7555,
    "lng": -97.3308
  },
  {
    "name": "Frisco",
    "label": "Frisco, TX",
    "lat": 33.1507,
    "lng": -96.8236
  },
  {
    "name": "Arlington",
    "label": "Arlington, TX",
    "lat": 32.7357,
    "lng": -97.1081
  },
  {
    "name": "Irving",
    "label": "Irving, TX",
    "lat": 32.814,
    "lng": -96.9489
  },
  {
    "name": "Denton",
    "label": "Denton, TX",
    "lat": 33.2148,
    "lng": -97.1331
  },
  {
    "name": "McKinney",
    "label": "McKinney, TX",
    "lat": 33.1972,
    "lng": -96.6398
  },
  {
    "name": "Grapevine",
    "label": "Grapevine, TX",
    "lat": 32.9343,
    "lng": -97.0781
  },
  {
    "name": "Lewisville",
    "label": "Lewisville, TX",
    "lat": 33.0462,
    "lng": -96.9942
  },
  {
    "name": "Carrollton",
    "label": "Carrollton, TX",
    "lat": 32.9537,
    "lng": -96.8903
  },
  {
    "name": "Richardson",
    "label": "Richardson, TX",
    "lat": 32.9483,
    "lng": -96.7299
  },
  {
    "name": "Grand Prairie",
    "label": "Grand Prairie, TX",
    "lat": 32.746,
    "lng": -96.9978
  },
  {
    "name": "Allen",
    "label": "Allen, TX",
    "lat": 33.1032,
    "lng": -96.6706
  },
  {
    "name": "The Colony",
    "label": "The Colony, TX",
    "lat": 33.0806,
    "lng": -96.8928
  }
];

export const LISTINGS = [
  {
    "id": "dma-free-sunday",
    "kind": "event",
    "city": "Dallas",
    "venue": "Dallas Museum of Art",
    "address": "1717 N Harwood St, Dallas, TX 75201",
    "lat": 32.787563,
    "lng": -96.801024,
    "start": "2026-09-06T11:00:00-05:00",
    "end": "2026-09-06T17:00:00-05:00",
    "costType": "free",
    "category": "arts",
    "attributes": [],
    "image": "assets/listings/dma-free-sunday-8118b1fb320c.webp",
    "actionUrl": "https://dma.org/visit/DMA-Free-First-Sundays",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "8118b1fb320ca0555c8ad850269838aeb786ee543944a3c6ce37e64d4c19dd33",
      "assetHash": "be92a480b284915200b6ca707c2589b77475348aadf5b4a4925208c78a8419f3",
      "brief": {
        "venueArchetype": "a spacious contemporary art museum gallery with a public open-studio table",
        "primaryActivity": "visitors exploring exhibitions while a family joins a hands-on art activity",
        "heroElement": "a child and adult creating a colorful abstract paper artwork with a gallery receding behind them",
        "audience": "a calm mixed-age group of museum visitors",
        "time": "midday",
        "season": "early September",
        "composition": "eye-level wide scene with the art table in the foreground and gallery depth on a diagonal",
        "lighting": "bright diffuse skylight with soft museum track lighting",
        "palette": "gallery white, pale oak, cobalt, coral, and small acid-green accents",
        "avoid": [
          "recognizable artworks",
          "exact museum architecture",
          "a private classroom",
          "crowded blockbuster exhibition"
        ]
      }
    },
    "title": "DMA 무료 첫째 일요일: Access for All",
    "kicker": "9월 6일 · 특별전 포함",
    "dateLabel": "9월 6일 일요일 · 오전 11시–오후 5시",
    "cost": "특별전까지 무료",
    "tags": [
      "예술",
      "특별전",
      "오픈 스튜디오",
      "투어"
    ],
    "summary": "DMA의 9월 무료 개방일에는 전시를 무료로 보고, 정오부터 오후 4시까지 오픈 스튜디오와 오후 2시 컬렉션 하이라이트 투어에도 참여할 수 있어요.",
    "highlights": [
      "현재 전시를 입장료 없이 관람",
      "정오–오후 4시 오픈 스튜디오 참여",
      "오후 2시 컬렉션 하이라이트 투어 포함"
    ],
    "finePrint": "나이와 관계없이 모든 방문객은 무료 입장권이 필요해요. 박물관 지하 주차장 이용료는 $20이며 무료 혜택에 포함되지 않아요.",
    "reservation": "무료 입장권 필요 · 온라인 예약 또는 현장 발권",
    "actionLabel": "무료 개방일 확인",
    "imageAlt": "미술관 오픈 스튜디오에서 가족이 작품을 만드는 모습을 표현한 서비스용 이미지",
    "cardHook": "특별전부터 오픈 스튜디오·투어까지 하루 동안 모두 무료예요.",
    "overview": "DMA의 9월 무료 개방일에는 전시를 무료로 보고, 정오부터 오후 4시까지 오픈 스튜디오와 오후 2시 컬렉션 하이라이트 투어에도 참여할 수 있어요.",
    "description": [
      "오전 11시부터 오후 5시까지 현재 전시를 무료로 관람할 수 있습니다. 정오부터 오후 4시까지 이어지는 오픈 스튜디오에서는 전시 관람에 참여형 활동을 더할 수 있어요.",
      "오후 2시에는 컬렉션 하이라이트 투어가 열립니다. 전시, 만들기 활동, 투어를 한 번에 선택해 즐길 수 있는 무료 개방일입니다."
    ],
    "practicalTips": [
      "무료 입장권은 온라인으로 예약하거나 현장에서 받을 수 있어요."
    ],
    "titleBase": "DMA 무료 첫째 일요일: Access for All",
    "verifiedAt": "2026-08-02T18:33:41.000Z",
    "sources": [
      {
        "name": "Dallas Museum of Art",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://dma.org/visit/DMA-Free-First-Sundays",
        "official": true
      },
      {
        "name": "Dallas Museum of Art",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://dma.org/visit",
        "official": true
      },
      {
        "name": "Dallas Museum of Art",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://dma.org/visit?gad_source=1",
        "official": true
      }
    ]
  },
  {
    "id": "carrollton-culture-fest",
    "kind": "event",
    "city": "Carrollton",
    "venue": "Trinity Mills Station Esplanade",
    "address": "1450 Sunrise Ave, Carrollton, TX 75006",
    "lat": 32.98128,
    "lng": -96.92415,
    "start": "2026-09-26T15:00:00-05:00",
    "end": "2026-09-26T21:00:00-05:00",
    "costType": "free",
    "category": "festival",
    "attributes": [],
    "image": "assets/listings/carrollton-culture-fest-4517d0feb5ec.webp",
    "actionUrl": "https://www.cityofcarrollton.com/departments/departments-g-p/parks-recreation/programs-events/downtown-carrollton/carrollton-culture-fest",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "4517d0feb5ecfc82994c2b7ae6ed8f374038c2542a0b6150cdc34cad6eb6d60c",
      "assetHash": "ce2c640bffe6a46784010da675789788456dcb6d6b2e3f0458ec4cb5a9734ea9",
      "brief": {
        "venueArchetype": "a modern transit-adjacent public esplanade arranged for an international community festival",
        "primaryActivity": "families exploring globally inspired crafts, food vendors, and a live cultural performance",
        "heroElement": "a parent and child making a colorful woven craft while dancers perform on a small stage behind",
        "audience": "a diverse multigenerational community crowd",
        "time": "late afternoon",
        "season": "late September",
        "composition": "medium-wide view through a craft booth with the family foreground and performance framed in the distance",
        "lighting": "clear warm afternoon light with colorful canopy reflections",
        "palette": "turquoise, saffron, berry, leaf green, and warm concrete",
        "avoid": [
          "national stereotypes",
          "dominant flags",
          "free-food implication",
          "exact station architecture"
        ]
      }
    },
    "title": "캐럴턴 컬처 페스트",
    "kicker": "9월 26일 · 트리니티 밀스",
    "dateLabel": "9월 26일 토요일 · 오후 3시–9시",
    "cost": "입장 무료",
    "tags": [
      "문화",
      "음식",
      "라이브 공연",
      "가족"
    ],
    "summary": "트리니티 밀스역 광장에서 여러 나라의 음식과 라이브 공연, 세계 각지의 공예품 판매 부스와 어린이 공예 체험을 만날 수 있는 문화 축제예요.",
    "highlights": [
      "여러 문화권의 라이브 공연",
      "세계 각국의 음식·공예 판매 부스",
      "어린이 공예와 축제 입장 무료"
    ],
    "finePrint": "축제 입장은 무료지만 음식·음료·공예품·기념품은 따로 구매해야 해요. 공연 시간과 주차·도로 통제 내용은 공식 페이지에 추후 안내될 수 있어요.",
    "reservation": "사전 예약 없이 바로 입장",
    "actionLabel": "최신 축제 안내 보기",
    "imageAlt": "야외 문화 축제에서 가족이 공예 부스와 라이브 공연을 즐기는 모습을 표현한 서비스용 이미지",
    "cardHook": "여러 나라의 음식·공연·공예를 무료입장 축제에서 한꺼번에 만나요.",
    "overview": "트리니티 밀스역 광장에서 여러 나라의 음식과 라이브 공연, 세계 각지의 공예품 판매 부스와 어린이 공예 체험을 만날 수 있는 문화 축제예요.",
    "description": [
      "오후 3시부터 9시까지 트리니티 밀스역 광장이 문화 축제 공간으로 바뀝니다. 라이브 공연을 보고, 세계 각지에서 영감을 받은 공예품 판매 부스와 어린이 공예 체험을 둘러볼 수 있어요."
    ],
    "practicalTips": [
      "사전 입장권이나 등록 없이 현장에서 입장할 수 있어요."
    ],
    "titleBase": "캐럴턴 컬처 페스트",
    "verifiedAt": "2026-08-02T18:33:41.000Z",
    "sources": [
      {
        "name": "City of Carrollton",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://cityofcarrollton.com/departments/departments-g-p/parks-recreation/programs-events/downtown-carrollton/carrollton-culture-fest",
        "official": true
      },
      {
        "name": "City of Carrollton",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://cityofcarrollton.com/departments/departments-g-p/parks-recreation/programs-events/trinity-nights",
        "official": true
      }
    ]
  },
  {
    "id": "cottonwood-art-festival-fall",
    "kind": "event",
    "city": "Richardson",
    "venue": "Cottonwood Park",
    "address": "1321 W Belt Line Rd, Richardson, TX 75080",
    "lat": 32.951165,
    "lng": -96.764079,
    "start": "2026-10-03T10:00:00-05:00",
    "end": "2026-10-04T17:00:00-05:00",
    "costType": "free",
    "category": "arts",
    "attributes": [],
    "image": "assets/listings/cottonwood-art-festival-fall-d54ccda25c1f.webp",
    "actionUrl": "https://cottonwoodartfestival.com/information/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "d54ccda25c1fa0a9a5c346fecb7c5a63f3414c1630dfbfca3d39063fa332b69c",
      "assetHash": "886f496ac34c8b4ebe09e0cfe67d5c40cd9aa8010256332f67c01d784a6358dc",
      "brief": {
        "venueArchetype": "a shaded urban park lined with airy white juried-art booths",
        "primaryActivity": "visitors meeting artists, viewing original work, and children joining a hands-on art area",
        "heroElement": "an artist presenting a textured ceramic piece to a family beside a display of abstract handmade work",
        "audience": "artists, families, and casual adult art shoppers",
        "time": "bright late morning",
        "season": "early October fall",
        "composition": "eye-level medium shot inside the edge of an art booth with the park festival softly layered behind",
        "lighting": "dappled tree light softened by white booth fabric",
        "palette": "ivory canvas, moss green, clay orange, cobalt glaze, and warm oak",
        "avoid": [
          "recognizable copyrighted artworks",
          "gallery interior",
          "mass-produced crafts",
          "exact artist count"
        ]
      }
    },
    "title": "코튼우드 아트 페스티벌",
    "kicker": "10월 3–4일 · 이틀간",
    "dateLabel": "10월 3일 토요일 오전 10시–오후 7시 · 4일 일요일 오전 10시–오후 5시",
    "cost": "입장 무료",
    "tags": [
      "예술",
      "가족",
      "야외",
      "페스티벌"
    ],
    "summary": "코튼우드 공원에서 심사를 거쳐 선정된 작가들의 작품과 푸드코트, 어린이 체험 공간 ArtStop을 둘러볼 수 있는 이틀간의 가을 축제예요.",
    "highlights": [
      "심사를 거쳐 선정된 작가들이 참여하는 이틀간의 축제",
      "작품 부스와 푸드 코트",
      "어린이 ArtStop 운영·일부 체험은 소액 유료"
    ],
    "finePrint": "축제 입장은 무료지만 ArtStop의 일부 어린이 체험에는 소액의 참가비가 있어요. 음식과 작품 구매도 별도예요.",
    "reservation": "사전 예약 없이 바로 입장",
    "actionLabel": "공식 축제 정보 확인",
    "imageAlt": "가을 공원 미술 축제에서 작가와 가족이 작품을 살펴보는 모습을 표현한 서비스용 이미지",
    "cardHook": "심사 선정 작가 작품과 어린이 ArtStop을 이틀간 무료입장으로 둘러봐요.",
    "overview": "코튼우드 공원에서 심사를 거쳐 선정된 작가들의 작품과 푸드코트, 어린이 체험 공간 ArtStop을 둘러볼 수 있는 이틀간의 가을 축제예요.",
    "description": [
      "토요일은 오전 10시부터 오후 7시, 일요일은 오전 10시부터 오후 5시까지 열립니다. 코튼우드 공원에서 작가들의 작품, 푸드코트, 어린이 체험 구역을 한 동선에서 둘러볼 수 있어요."
    ],
    "practicalTips": [
      "축제 입장은 사전 예약 없이 가능해요."
    ],
    "titleBase": "코튼우드 아트 페스티벌",
    "verifiedAt": "2026-08-02T18:33:41.000Z",
    "sources": [
      {
        "name": "Cottonwood Art Festival and City of Richardson",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://cottonwoodartfestival.com/information",
        "official": true
      },
      {
        "name": "Cottonwood Art Festival and City of Richardson",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://cottonwoodartfestival.com/artstop",
        "official": true
      },
      {
        "name": "City of Richardson",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://cor.net/Home/Components/FacilityDirectory/FacilityDirectory/68",
        "official": true
      }
    ]
  },
  {
    "id": "the-colony-family-fright-fest",
    "kind": "event",
    "city": "The Colony",
    "venue": "Stewart Creek Park",
    "address": "3700 Sparks Rd, The Colony, TX 75056",
    "lat": 33.086257,
    "lng": -96.919236,
    "start": "2026-10-24T15:00:00-05:00",
    "end": "2026-10-24T19:00:00-05:00",
    "costType": "free",
    "category": "family",
    "attributes": [],
    "image": "assets/listings/the-colony-family-fright-fest-a94ba944ba23.webp",
    "actionUrl": "https://www.thecolonytx.gov/835/Family-Fright-Fest",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "a94ba944ba236485aee401004b233185091fb834bd1d945b00a56aff7a215efb",
      "assetHash": "0361400f82e1a8cf809455b389bb79f98d055fd42a165e80c93463ac8b6438f8",
      "brief": {
        "venueArchetype": "a friendly lakeside North Texas park prepared for an afternoon family Halloween event",
        "primaryActivity": "children moving between crafts, trick-or-treat booths, games, and a hay wagon",
        "heroElement": "two children in original homemade costumes decorating small treat bags with a hay wagon behind",
        "audience": "families with elementary-age children in playful non-scary costumes",
        "time": "golden late afternoon",
        "season": "late October",
        "composition": "low child-height medium-wide angle with the craft table foreground and lake-and-hay-wagon depth",
        "lighting": "warm autumn sun with soft shaded faces",
        "palette": "pumpkin orange, lake blue, straw gold, plum, and soft green",
        "avoid": [
          "copyrighted costumes",
          "horror imagery",
          "nighttime darkness",
          "vehicle-entry implication"
        ]
      }
    },
    "title": "패밀리 프라이트 페스트",
    "kicker": "10월 24일 · 가을 가족 축제",
    "dateLabel": "10월 24일 토요일 · 오후 3시–7시",
    "cost": "도보 입장·체험 무료",
    "tags": [
      "가족",
      "할로윈",
      "코스튬",
      "야외"
    ],
    "summary": "스튜어트 크리크 공원에서 헤이라이드(마차 타기), 게임, 트릭 오어 트리트 부스, 공예, 연령대별 코스튬 콘테스트를 즐길 수 있어요. 행사 체험은 무료예요.",
    "highlights": [
      "헤이라이드·게임·트릭 오어 트리트·공예",
      "연령대별 코스튬 콘테스트",
      "도보 입장과 행사 체험 무료"
    ],
    "finePrint": "공원 밖에 주차한 뒤 도보로 입장하면 무료예요. 차량으로 공원 안에 들어갈 경우 차량당 $15이고, 연간 공원 패스가 있으면 차량 입장도 무료예요. 음식은 별도 구매예요.",
    "reservation": "사전 예약 없음 · 차량 입장료는 현장 결제",
    "actionLabel": "공식 이용 조건 확인",
    "imageAlt": "가을 호숫가 공원에서 아이들이 공예와 트릭 오어 트리트를 즐기는 모습의 서비스용 이미지",
    "cardHook": "헤이라이드·게임·트릭 오어 트리트와 코스튬 콘테스트를 무료로 즐겨요.",
    "overview": "스튜어트 크리크 공원에서 헤이라이드(마차 타기), 게임, 트릭 오어 트리트 부스, 공예, 연령대별 코스튬 콘테스트를 즐길 수 있어요. 행사 체험은 무료예요.",
    "description": [
      "오후 3시부터 7시까지 스튜어트 크리크 공원에서 헤이라이드, 게임, 공예와 트릭 오어 트리트 부스가 이어집니다. 코스튬 콘테스트는 0–3세부터 13세 이상까지 연령대별로 진행돼요."
    ],
    "practicalTips": [
      "연간 공원 패스가 있으면 차량으로도 무료 입장할 수 있어요."
    ],
    "titleBase": "패밀리 프라이트 페스트",
    "verifiedAt": "2026-08-02T18:33:41.000Z",
    "sources": [
      {
        "name": "City of The Colony",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://thecolonytx.gov/835/Family-Fright-Fest",
        "official": true
      },
      {
        "name": "City of The Colony",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://thecolonytx.gov/calendar.aspx?EID=5059",
        "official": true
      }
    ]
  },
  {
    "id": "grand-prairie-main-street-fest",
    "kind": "event",
    "city": "Grand Prairie",
    "venue": "Downtown Grand Prairie",
    "address": "300 W Main St, Grand Prairie, TX 75050",
    "lat": 32.74569,
    "lng": -97.0067,
    "start": "2026-10-23T17:00:00-05:00",
    "end": "2026-10-25T20:00:00-05:00",
    "costType": "free",
    "category": "festival",
    "attributes": [],
    "image": "assets/listings/grand-prairie-main-street-fest-6011645992ad.webp",
    "actionUrl": "https://www.gptx.org/Community-Calendar/Main-Street-Fest-2026",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "6011645992ad515a0a8c41cf2b8ff80edb9fb54ae913061288dc787d6f9df4cf",
      "assetHash": "edb29704d04ec61e8ce74745eea2889f47f1c06888b80937bd77cd308a36d333",
      "brief": {
        "venueArchetype": "a civic downtown main street closed for a broad three-day community festival",
        "primaryActivity": "families walking between a regional live-music stage, a kids area, and local vendor tents",
        "heroElement": "a family moving toward a glowing small concert stage while a child carries a simple festival craft",
        "audience": "a lively mixed-age regional crowd",
        "time": "early evening just after sunset",
        "season": "late October",
        "composition": "wide low street perspective with vendor lights forming leading lines toward the offset stage",
        "lighting": "deep blue dusk balanced by warm string and stage lights",
        "palette": "midnight blue, electric coral, amber, turquoise, and warm pavement",
        "avoid": [
          "exact city hall architecture",
          "specific bands",
          "carnival rides",
          "readable vendor signs"
        ]
      }
    },
    "title": "그랜드프레리 메인 스트리트 페스트",
    "kicker": "10월 23–25일 · 입장과 주차 무료",
    "dateLabel": "10월 23일 금요일 오후 5시–자정 · 24일 오전 10시–자정 · 25일 정오–오후 8시",
    "cost": "입장·행사 주차 무료",
    "tags": [
      "페스티벌",
      "가족",
      "라이브 음악",
      "다운타운"
    ],
    "summary": "그랜드프레리 시청 일대에서 라이브 음악과 지역 밴드 공연, 키즈존, 다양한 판매 부스를 즐길 수 있는 사흘간의 도심 축제예요.",
    "highlights": [
      "사흘간 이어지는 라이브 음악과 지역 밴드",
      "키즈존과 각종 판매 부스",
      "축제 입장과 행사 주차 무료"
    ],
    "finePrint": "입장과 행사 주차는 무료예요. 음식·음료·놀이기구·판매 부스 이용에는 비용이 들 수 있고, 세부 프로그램은 나중에 바뀔 수 있어요.",
    "reservation": "사전 예약 없이 바로 입장",
    "actionLabel": "공식 축제 일정 확인",
    "imageAlt": "저녁 도심 거리 축제에서 가족이 라이브 음악과 부스를 즐기는 모습의 서비스용 이미지",
    "cardHook": "라이브 음악과 키즈존을 사흘간 무료입장·무료 주차로 즐겨요.",
    "overview": "그랜드프레리 시청 일대에서 라이브 음악과 지역 밴드 공연, 키즈존, 다양한 판매 부스를 즐길 수 있는 사흘간의 도심 축제예요.",
    "description": [
      "금요일 저녁부터 일요일 저녁까지 그랜드프레리 시청 일대에서 열리는 도심 축제입니다. 지역 밴드와 라이브 음악, 키즈존, 여러 판매 부스를 날짜별 운영시간에 맞춰 둘러볼 수 있어요."
    ],
    "practicalTips": [
      "행사 주차는 무료로 안내되어 있어요."
    ],
    "titleBase": "그랜드프레리 메인 스트리트 페스트",
    "verifiedAt": "2026-08-02T18:33:41.000Z",
    "sources": [
      {
        "name": "City of Grand Prairie",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://gptx.org/Community-Calendar/Main-Street-Fest-2026",
        "official": true
      },
      {
        "name": "City of Grand Prairie",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://gptx.org/About-Grand-Prairie/Downtown-Grand-Prairie",
        "official": true
      },
      {
        "name": "Grand Fun GP",
        "note": "독립 출처에서 핵심 사실 교차 확인",
        "url": "https://grandfungp.com/ko/special-events",
        "official": false
      }
    ]
  },
  {
    "id": "allen-pass-play-expo",
    "kind": "event",
    "city": "Allen",
    "venue": "Stephen G. Terrell Recreation Center",
    "address": "1680 W Exchange Pkwy, Allen, TX 75013",
    "lat": 33.117577,
    "lng": -96.714826,
    "start": "2026-09-01T18:00:00-05:00",
    "end": "2026-09-01T20:00:00-05:00",
    "costType": "discount",
    "category": "fitness",
    "attributes": [],
    "image": "assets/listings/allen-pass-play-expo-0cd7e11e1003.webp",
    "actionUrl": "https://events.allentx.gov/allentx/260197823",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "0cd7e11e1003e5dc54f19fd30ecabd601c6acf91b2d6e4fd7f574ae3bbd7e5a5",
      "assetHash": "e635e56fadba59c19997c65041b633174d8e9a74fce62a5c374c6add27ad8bba",
      "brief": {
        "venueArchetype": "a bright modern municipal recreation center with connected fitness and class spaces",
        "primaryActivity": "visitors touring the facility and sampling the atmosphere of fall arts, movement, and fitness programs",
        "heroElement": "a parent and child entering an activity zone as an instructor demonstrates a simple movement exercise",
        "audience": "families, teens, and adults exploring the recreation center",
        "time": "early evening",
        "season": "early September fall registration period",
        "composition": "dynamic wide interior view with the family foreground and distinct activity zones layered through glass behind",
        "lighting": "bright architectural daylight fading into crisp indoor evening light",
        "palette": "sport blue, lime, pale maple, charcoal, and warm coral",
        "avoid": [
          "readable discount signs",
          "implying every class is in session",
          "professional sports arena",
          "specific facility architecture"
        ]
      }
    },
    "title": "패스 & 플레이 엑스포 — 가을 강좌 30% 할인",
    "kicker": "9월 1일 · 시설 무료 체험",
    "dateLabel": "9월 1일 화요일 · 오후 6시–8시",
    "cost": "입장 무료 · 가을 강좌 30% 할인",
    "tags": [
      "할인",
      "피트니스",
      "전 연령",
      "체험"
    ],
    "summary": "앨런 레크리에이션 센터를 무료로 둘러보고 예술·음악·유아 프로그램·양궁·댄스·무술·피트니스 등 가을 강좌를 알아보는 행사예요. 행사 참석자는 가을 강좌 등록비를 30% 할인받을 수 있어요.",
    "highlights": [
      "레크리에이션 센터와 프로그램을 무료로 둘러보기",
      "예술·음악·양궁·댄스·무술·피트니스 수업 안내",
      "행사 참석자는 가을 강좌 등록비 30% 할인"
    ],
    "finePrint": "30% 할인은 행사 참석자가 가을 강좌에 등록할 때 적용돼요. 강좌별 남은 자리와 다른 등록 조건은 공식 페이지에서 확인해 주세요.",
    "reservation": "행사 사전 신청 없음",
    "actionLabel": "공식 행사 정보 확인",
    "imageAlt": "현대적인 레크리에이션 센터에서 가족이 여러 활동 공간을 둘러보는 모습의 서비스용 이미지",
    "cardHook": "레크리에이션 센터를 무료로 체험하고 가을 강좌 등록비도 30% 아껴요.",
    "overview": "앨런 레크리에이션 센터를 무료로 둘러보고 예술·음악·유아 프로그램·양궁·댄스·무술·피트니스 등 가을 강좌를 알아보는 행사예요. 행사 참석자는 가을 강좌 등록비를 30% 할인받을 수 있어요.",
    "description": [
      "오후 6시부터 8시까지 센터를 둘러보며 예술, 음악, 유아 프로그램, 양궁, 댄스, 무술과 피트니스 강좌를 확인할 수 있습니다. 행사 참석자가 가을 강좌에 등록하면 등록비 30% 할인이 적용돼요."
    ],
    "practicalTips": [
      "행사 참여에는 사전 등록이 필요하지 않아요."
    ],
    "titleBase": "패스 & 플레이 엑스포",
    "titleBenefit": "가을 강좌 30% 할인",
    "verifiedAt": "2026-08-02T18:33:41.000Z",
    "sources": [
      {
        "name": "City of Allen",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://events.allentx.gov/allentx/260197823",
        "official": true
      },
      {
        "name": "City of Allen",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://events.allentx.gov/allentx/260128876",
        "official": true
      },
      {
        "name": "Mapcarta with OpenStreetMap data",
        "note": "독립 출처에서 핵심 사실 교차 확인",
        "url": "https://mapcarta.com/W449240967",
        "official": false
      }
    ]
  },
  {
    "id": "action-park-grand-prairie-bogo-skate",
    "kind": "benefit",
    "city": "Grand Prairie",
    "venue": "Action Park Grand Prairie",
    "address": "1002 Lone Star Pkwy, Grand Prairie, TX 75050",
    "lat": 32.7733222,
    "lng": -96.9933897,
    "start": "2026-08-07T00:00:00-05:00",
    "end": "2026-12-31T23:59:00-06:00",
    "costType": "discount",
    "category": "local-deals",
    "attributes": [],
    "actionUrl": "https://www.visitgrandprairietx.com/Deals",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "a4fca583b6fa1d0fd1f53a5e68f9e820d90d8d7e71e11d40aa2b2649a025e0a2",
      "assetHash": "324dd102b3767327551aa0038eb823961fc8e125917fdcbf30e9fbd5dab267de",
      "brief": {
        "venueArchetype": "a lively but unbranded indoor roller rink with a polished floor and soft perimeter lights",
        "primaryActivity": "two visitors enjoying a roller-skating session together",
        "heroElement": "two pairs of skaters gliding side by side through a gentle turn",
        "audience": "a small mixed-age group of recreational skaters",
        "time": "early evening",
        "season": "late summer",
        "composition": "low eye-level wide scene following the curve of the rink with the paired skaters centered",
        "lighting": "warm practical rink lighting with soft colored reflections on the floor",
        "palette": "charcoal, warm amber, muted violet, teal, and natural denim",
        "avoid": [
          "readable coupons",
          "logos",
          "competitive speed skating",
          "an empty rink"
        ]
      }
    },
    "image": "assets/listings/action-park-grand-prairie-bogo-skate-a4fca583b6fa.webp",
    "title": "액션 파크 그랜드 프레리 — 스케이트 세션 1+1",
    "dateLabel": "2026년 12월 31일까지",
    "cost": "스케이트 1+1 또는 매장 구매 10% 할인",
    "summary": "공식 방문자 쿠폰으로 스케이트 세션 1+1 또는 매장 구매 10% 할인 중 하나를 선택할 수 있습니다.",
    "finePrint": "고객당 쿠폰 1장, 180일마다 1회 사용이며 매장이 쿠폰을 변경하거나 거절할 수 있어 방문 전 확인이 필요합니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "실내 롤러스케이트장에서 두 사람이 함께 스케이트를 즐기는 모습을 표현한 서비스용 이미지",
    "cardHook": "스케이트 세션 1+1과 매장 구매 10% 할인 중 하나를 골라요.",
    "overview": "공식 방문자 쿠폰으로 스케이트 세션 1+1 또는 매장 구매 10% 할인 중 하나를 선택할 수 있습니다.",
    "description": [
      "공식 방문자 쿠폰은 2026년 12월 31일까지 사용할 수 있습니다. 스케이트 세션 하나를 구매하면 하나를 무료로 받거나, 매장 구매 금액의 10%를 할인받는 두 선택지 중 하나를 이용해요."
    ],
    "highlights": [
      "스케이트 세션 구매 시 두 번째 세션 무료",
      "대신 매장 구매 10% 할인을 선택 가능",
      "고객당 180일마다 쿠폰 1회 사용"
    ],
    "practicalTips": [
      "방문 전에 매장이 쿠폰을 계속 받는지 확인해 주세요."
    ],
    "titleBase": "액션 파크 그랜드 프레리",
    "titleBenefit": "스케이트 세션 1+1",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "쿠폰 조건을 충족하는 일반 고객에게 적용됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Visit Grand Prairie",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://visitgrandprairietx.com/Deals",
        "official": true
      }
    ]
  },
  {
    "id": "african-american-museum-dallas-free-admission",
    "kind": "benefit",
    "city": "Dallas",
    "venue": "African American Museum Dallas",
    "address": "3536 Grand Ave, Dallas, TX 75210",
    "lat": 32.779055,
    "lng": -96.764444,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "free",
    "category": "culture",
    "attributes": [],
    "actionUrl": "https://aamdallas.org/plan-your-visit/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "33230c137d670804bb8bfef6e90c70728d827bc3ec3ac1af160f70d92047dfe3",
      "assetHash": "ba3318f78f9e05c2e0a7f0e49cca143b8208766693d33cbe2c27fd8b2d33452a",
      "brief": {
        "venueArchetype": "a dignified cultural-history museum gallery with archival-style displays and sculptural forms",
        "primaryActivity": "individual visitors thoughtfully exploring African American history and cultural exhibits",
        "heroElement": "an adult and teenager pausing together before a layered artifact display",
        "audience": "a few independent museum visitors",
        "time": "early afternoon",
        "season": "late summer",
        "composition": "quiet eye-level medium-wide gallery view with the visitors in profile and deep exhibit layers",
        "lighting": "controlled museum spot lighting with soft ambient daylight",
        "palette": "deep indigo, warm walnut, bronze, cream, and muted red",
        "avoid": [
          "recognizable artworks",
          "readable exhibit text",
          "exact museum architecture",
          "crowded tour groups"
        ]
      }
    },
    "image": "assets/listings/african-american-museum-dallas-free-admission-33230c137d67.webp",
    "title": "아프리칸 아메리칸 뮤지엄 댈러스 — 일반 입장 무료",
    "dateLabel": "상시 · 화–금 오전 11시–오후 5시, 토 오전 10시–오후 5시",
    "cost": "일반 입장 무료",
    "summary": "댈러스 페어 파크의 박물관을 일반 방문객은 무료로 관람할 수 있습니다.",
    "finePrint": "10명을 초과하는 단체 투어와 일부 특별 행사·전시는 별도 조건이 적용됩니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "박물관 관람객이 아프리카계 미국인의 역사와 문화를 다룬 전시를 살펴보는 모습을 표현한 서비스용 이미지",
    "cardHook": "페어 파크의 아프리칸 아메리칸 뮤지엄을 일반 관람은 무료로 둘러봐요.",
    "overview": "댈러스 페어 파크의 박물관을 일반 방문객은 무료로 관람할 수 있습니다.",
    "description": [
      "일반 관람은 화요일–금요일 오전 11시부터 오후 5시, 토요일 오전 10시부터 오후 5시까지 가능합니다. 개별 방문객은 무료로 입장하며 일반 관람에는 예약이 필요하지 않아요."
    ],
    "highlights": [
      "개별 방문객 일반 입장 무료",
      "화–금 오전 11시–오후 5시 · 토 오전 10시–오후 5시",
      "일반 관람은 예약 없이 이용 가능"
    ],
    "practicalTips": [
      "일반 관람은 예약 없이 방문할 수 있어요."
    ],
    "titleBase": "아프리칸 아메리칸 뮤지엄 댈러스",
    "titleBenefit": "일반 입장 무료",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "개별 방문객에게 적용되며 10명을 초과하는 단체 투어는 예외입니다."
    },
    "booking": {
      "mode": "not-required",
      "detail": "일반 관람은 예약이 필요 없으며 특별 행사·전시는 티켓이 필요할 수 있습니다."
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "African American Museum Dallas",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://aamdallas.org/plan-your-visit",
        "official": true
      }
    ]
  },
  {
    "id": "allen-leap-days-2026",
    "kind": "benefit",
    "city": "Allen",
    "venue": "Participating Allen recreation centers",
    "address": "Multiple locations, Allen, TX",
    "lat": 33.1032,
    "lng": -96.6706,
    "start": "2026-02-01T00:00:00-06:00",
    "end": "2026-12-01T23:59:00-06:00",
    "costType": "free",
    "category": "recreation",
    "attributes": [],
    "actionUrl": "https://events.allentx.gov/allentx/260128874",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "926245ef28e6fba41c3c4a7a566d0c157ab6aabdfefa443cae6295e5bb2386cf",
      "assetHash": "62a10aaf10882a7c02f19bacb9b158791375bf9eab336f3c53d7c0300096c9ed",
      "brief": {
        "venueArchetype": "a bright multi-use community recreation center atrium with glimpses of gym and pool activity zones",
        "primaryActivity": "a family arriving for a free day of recreation-center access",
        "heroElement": "a parent and two children stepping through the open atrium toward the activity areas",
        "audience": "families and general community visitors",
        "time": "mid-morning",
        "season": "an ordinary first day of the month",
        "composition": "high-corner wide view with the arriving family moving diagonally through the spacious atrium",
        "lighting": "crisp skylight with cool reflected pool light and warm wood accents",
        "palette": "aqua, pale maple, white, sunflower yellow, and navy",
        "avoid": [
          "event booths",
          "conference staging",
          "exact facility architecture",
          "readable signs"
        ]
      }
    },
    "image": "assets/listings/allen-leap-days-2026-926245ef28e6.webp",
    "title": "앨런 Leap Days — 매월 1일 참여 시설 입장 무료",
    "dateLabel": "2026년 12월까지 · 매월 1일",
    "cost": "참여 레크리에이션 센터 입장 무료",
    "summary": "매월 1일 앨런의 참여 레크리에이션 센터를 무료로 이용할 수 있습니다.",
    "finePrint": "첫날이 시설 휴관일이나 시 공휴일이면 참여하지 않으며 시설별 연령·이용 규정은 유지됩니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "가족이 현대적인 커뮤니티 레크리에이션 센터에 들어서는 모습을 표현한 서비스용 이미지",
    "cardHook": "매월 1일, 앨런의 참여 레크리에이션 센터를 입장료 없이 이용해요.",
    "overview": "매월 1일 앨런의 참여 레크리에이션 센터를 무료로 이용할 수 있습니다.",
    "description": [
      "2026년 12월 1일까지 매달 첫날에 참여 시설의 입장료가 무료입니다. 시설 휴관일이나 시 공휴일과 겹치면 참여하지 않으며, 각 센터의 기존 연령·이용 규정은 그대로 적용돼요."
    ],
    "highlights": [
      "매월 1일 참여 시설 입장 무료",
      "2026년 12월 1일까지 이어지는 혜택",
      "시설별 기존 연령·이용 규정은 유지"
    ],
    "practicalTips": [],
    "titleBase": "앨런 Leap Days",
    "titleBenefit": "매월 1일 참여 시설 입장 무료",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "참여 시설의 일반 이용 규정과 연령 제한은 그대로 적용됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "City of Allen",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://events.allentx.gov/allentx/260128874",
        "official": true
      }
    ]
  },
  {
    "id": "allen-show-your-badge-discounts",
    "kind": "benefit",
    "city": "Allen",
    "venue": "Participating Allen businesses",
    "address": "Multiple locations, Allen, TX",
    "lat": 33.1032,
    "lng": -96.6706,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "discount",
    "category": "local-deals",
    "attributes": [],
    "actionUrl": "https://visitallentexas.com/show-your-badge/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "a5bcfca1a765a903957c182bf490d4c881922629edfb2ec4331dae1318d67ae3",
      "assetHash": "4efc973a47bcf6ebe62b301fd9733bfbdef89ea9fc207fe409e8763205d11cea",
      "brief": {
        "venueArchetype": "a welcoming independent cafe and shop counter in a North Texas commercial district",
        "primaryActivity": "a conference attendee showing an unbranded event badge to receive a participating-business offer",
        "heroElement": "the attendee holding up a plain badge while a cashier gestures toward the purchase",
        "audience": "one adult visitor and a local business employee",
        "time": "late morning",
        "season": "year-round",
        "composition": "intimate over-the-counter three-quarter view with the badge and friendly exchange as the focal point",
        "lighting": "soft storefront daylight with warm pendant-light accents",
        "palette": "terracotta, cream, sage, espresso brown, and cobalt",
        "avoid": [
          "readable badge text",
          "company logos",
          "specific discount numbers",
          "hotel convention halls"
        ]
      }
    },
    "image": "assets/listings/allen-show-your-badge-discounts-a5bcfca1a765.webp",
    "title": "앨런 Show Your Badge Program — 참여 매장 할인·무료 품목",
    "dateLabel": "상시 · 참여 매장별 조건 적용",
    "cost": "10–20% 할인, 1+1, 무료 품목 등",
    "summary": "컨퍼런스 배지를 제시하면 앨런의 여러 참여 식당·상점에서 할인, 1+1, 무료 품목 등 각기 다른 혜택을 받을 수 있습니다.",
    "finePrint": "매장마다 할인율, 제외 품목, 첫 방문 조건과 중복 제한이 다르므로 공식 목록에서 해당 매장 조건을 확인하세요.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "컨퍼런스 참가자가 지역 매장에서 명찰을 보여주고 혜택을 받는 모습을 표현한 서비스용 이미지",
    "cardHook": "컨퍼런스 배지 하나로 앨런 참여 매장의 할인·1+1·무료 품목을 확인해요.",
    "overview": "컨퍼런스 배지를 제시하면 앨런의 여러 참여 식당·상점에서 할인, 1+1, 무료 품목 등 각기 다른 혜택을 받을 수 있습니다.",
    "description": [
      "참여 식당과 상점마다 10–20% 할인, 1+1, 금액 할인, 일부 무료 품목처럼 서로 다른 혜택을 제공합니다. 매장에서 유효한 컨퍼런스 배지를 보여주고 해당 업체의 제외 조건을 확인해야 해요."
    ],
    "highlights": [
      "유효한 컨퍼런스 배지 제시 필요",
      "참여처별 10–20% 할인·1+1·무료 품목",
      "업체마다 제외 품목과 중복 제한이 다름"
    ],
    "practicalTips": [
      "방문할 업체의 개별 조건을 공식 참여 목록에서 먼저 확인하세요."
    ],
    "titleBase": "앨런 Show Your Badge Program",
    "titleBenefit": "참여 매장 할인·무료 품목",
    "eligibility": {
      "mode": "conditional",
      "segments": [
        "other"
      ],
      "detail": "참여 매장에서 유효한 컨퍼런스 배지를 제시하는 방문객에게 적용됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Visit Allen",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://visitallentexas.com/show-your-badge",
        "official": true
      }
    ]
  },
  {
    "id": "arlington-summer-music-series-2026",
    "kind": "benefit",
    "city": "Arlington",
    "venue": "Home Plate Restaurant at Texas Rangers Golf Club",
    "address": "701 Brown Blvd, Arlington, TX 76011",
    "lat": 32.77657281843156,
    "lng": -97.10308612004947,
    "start": "2026-08-14T18:00:00-05:00",
    "end": "2026-09-11T20:00:00-05:00",
    "costType": "free",
    "category": "concert",
    "attributes": [],
    "actionUrl": "https://www.arlingtontx.gov/Community-Calendar/Parks-Recreation-Culture-Events/Summer-Music-Series",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "4d42576d05842d83b13724825c34ba52c5ec27bd318fc26af6cba9370ae3678d",
      "assetHash": "9d7cf6e67fd202095acbe14a32d4751abc350fb3d75a0dfd9e7fa5b4a1d98ef8",
      "brief": {
        "venueArchetype": "an open-air restaurant patio beside a green public golf-course landscape",
        "primaryActivity": "local musicians performing for walk-in dinner guests with no cover charge",
        "heroElement": "an acoustic guitarist and vocalist performing beside relaxed patio tables",
        "audience": "a modest seated crowd of adult and family diners",
        "time": "Friday at sunset",
        "season": "late summer",
        "composition": "eye-level panoramic patio view with the musicians on one third and tables leading across the scene",
        "lighting": "golden-hour backlight shifting into warm string-light glow",
        "palette": "summer green, honey gold, brick red, sky blue, and charcoal",
        "avoid": [
          "exact restaurant architecture",
          "golf branding",
          "large concert stage",
          "readable menus"
        ]
      }
    },
    "image": "assets/listings/arlington-summer-music-series-2026-4d42576d0584.webp",
    "title": "알링턴 Summer Music Series — 금요일 라이브 음악 무료",
    "dateLabel": "8월 14일–9월 11일 금요일 · 오후 6시–8시",
    "cost": "커버 차지 없음",
    "summary": "Home Plate에서 금요일 저녁 지역 뮤지션의 라이브 공연을 커버 차지 없이 즐길 수 있습니다.",
    "finePrint": "회원권은 필요 없고 현장 입장도 가능하지만 좌석이 한정되어 전화 사전 등록을 권장합니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "초저녁 야외 레스토랑 파티오에서 관객이 라이브 음악을 즐기는 모습을 표현한 서비스용 이미지",
    "cardHook": "금요일 저녁, 회원권 없이 지역 뮤지션 공연을 무료로 들어요.",
    "overview": "Home Plate에서 금요일 저녁 지역 뮤지션의 라이브 공연을 커버 차지 없이 즐길 수 있습니다.",
    "description": [
      "8월 14일부터 9월 11일까지 남은 금요일 오후 6–8시에 Home Plate에서 공연이 열립니다. 골프클럽 회원권 없이 일반 관객이 이용할 수 있고 커버 차지도 없습니다."
    ],
    "highlights": [
      "남은 금요일 오후 6–8시 공연",
      "커버 차지 없이 무료",
      "골프클럽 회원권 없이 일반 관객 이용 가능"
    ],
    "practicalTips": [
      "좌석이 남아 있으면 현장 입장이 가능하지만 전화 사전 등록을 권장해요."
    ],
    "titleBase": "알링턴 Summer Music Series",
    "titleBenefit": "금요일 라이브 음악 무료",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "회원권 없이 일반 대중이 이용할 수 있습니다."
    },
    "booking": {
      "mode": "recommended",
      "detail": "자리가 있는 경우 현장 입장이 가능하지만 전화 사전 등록을 권장합니다."
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "City of Arlington",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://arlingtontx.gov/Community-Calendar/Parks-Recreation-Culture-Events/Summer-Music-Series",
        "official": true
      }
    ]
  },
  {
    "id": "carrollton-library-card-free",
    "kind": "benefit",
    "city": "Carrollton",
    "venue": "Carrollton Public Library locations",
    "address": "Multiple locations, Carrollton, TX",
    "lat": 32.9537,
    "lng": -96.8903,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "free",
    "category": "library",
    "attributes": [],
    "actionUrl": "https://www.cityofcarrollton.com/departments/departments-g-p/library/cards-account/library-cards",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "7ab5558a2d58aab1a1ee8c9cae0c6fcf2907886c0c6aefad8a974da9688007a6",
      "assetHash": "c9994bcc23a7166fde4bacfe47dae8956f6a0675ce87923d00b5f1a51f0cf089",
      "brief": {
        "venueArchetype": "a contemporary public-library service desk with bookshelves and a calm civic interior",
        "primaryActivity": "an eligible resident completing identity and address verification for a free library card",
        "heroElement": "a librarian passing a plain new card across the desk beside closed identification documents",
        "audience": "one adult resident and one library staff member",
        "time": "weekday afternoon",
        "season": "year-round",
        "composition": "close eye-level counter scene focused on the card handoff with softly receding shelves",
        "lighting": "even window daylight and neutral library task lighting",
        "palette": "pale oak, dusty blue, cream, burgundy, and soft gray",
        "avoid": [
          "readable personal information",
          "branded card designs",
          "children applying alone",
          "exact branch architecture"
        ]
      }
    },
    "image": "assets/listings/carrollton-library-card-free-7ab5558a2d58.webp",
    "title": "캐럴턴 공공도서관 카드 — 해당 지역 거주자 무료",
    "dateLabel": "상시 · 도서관 카드 신청",
    "cost": "Carrollton·Farmers Branch 거주자 무료",
    "summary": "Carrollton과 Farmers Branch 거주자는 신분과 주소를 확인해 도서관 카드를 무료로 발급받을 수 있습니다.",
    "finePrint": "현재 주소가 신분증과 다르면 추가 주소 증빙이 필요하며 비거주자 카드는 연 $40입니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "도서관 서비스 데스크에서 주민이 신분과 주소를 확인하고 카드를 받는 모습을 표현한 서비스용 이미지",
    "cardHook": "Carrollton·Farmers Branch 거주자는 신분과 주소 확인 후 도서관 카드를 무료로 받아요.",
    "overview": "Carrollton과 Farmers Branch 거주자는 신분과 주소를 확인해 도서관 카드를 무료로 발급받을 수 있습니다.",
    "description": [
      "현재 정부 발행 사진 신분증으로 신분과 주소를 확인해 신청합니다. 신분증 주소가 현재 주소와 다르면 추가 증빙이 필요하며, 미성년자는 부모나 법정 보호자와 함께 신청해야 해요."
    ],
    "highlights": [
      "Carrollton·Farmers Branch 거주자 무료",
      "정부 발행 사진 신분증과 필요시 주소 증빙",
      "미성년자는 부모 또는 법정 보호자와 신청"
    ],
    "practicalTips": [],
    "titleBase": "캐럴턴 공공도서관 카드",
    "titleBenefit": "해당 지역 거주자 무료",
    "eligibility": {
      "mode": "conditional",
      "segments": [
        "resident",
        "library-card"
      ],
      "detail": "Carrollton 또는 Farmers Branch 거주자에게 무료이며 신분·주소 증빙이 필요합니다."
    },
    "booking": {
      "mode": "required",
      "detail": "성인은 신청해야 하며 미성년자는 부모 또는 법정 보호자가 함께 신청해야 합니다."
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "City of Carrollton",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://cityofcarrollton.com/departments/departments-g-p/library/cards-account/library-cards",
        "official": true
      }
    ]
  },
  {
    "id": "dallas-arboretum-museums-for-all",
    "kind": "benefit",
    "city": "Dallas",
    "venue": "Dallas Arboretum and Botanical Garden",
    "address": "8525 Garland Rd, Dallas, TX 75218",
    "lat": 32.823056,
    "lng": -96.716944,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "discount",
    "category": "access",
    "attributes": [],
    "actionUrl": "https://www.dallasarboretum.org/museums-for-all/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "458743c1ede0ff7a38cd848bb3e9005d36ebe27b7a0074fb26fceb2552e1c459",
      "assetHash": "86b845a973f8f26bcc92ede7689aea46e2de36215dd741c50e98960a981f059d",
      "brief": {
        "venueArchetype": "a generic botanical-garden welcome gate opening toward richly planted daytime paths",
        "primaryActivity": "an eligible household purchasing reduced-price daytime admission on site",
        "heroElement": "an adult presenting a plain benefits card at a small outdoor welcome counter while family waits nearby",
        "audience": "a family group of four and one garden attendant",
        "time": "bright morning",
        "season": "late summer",
        "composition": "medium-wide side view balancing the card exchange in front with a flower-lined path beyond",
        "lighting": "clear diffused daylight under a shaded entry canopy",
        "palette": "leaf green, coral blossom, limestone, sky blue, and natural linen",
        "avoid": [
          "exact garden entrance",
          "readable card text",
          "ticket prices in image",
          "formal event decor"
        ]
      }
    },
    "image": "assets/listings/dallas-arboretum-museums-for-all-458743c1ede0.webp",
    "title": "댈러스 아보리텀 Museums for All — 1인 $3",
    "dateLabel": "상시 · 주간 입장",
    "cost": "1인 $3 · 최대 6명",
    "summary": "SNAP EBT 카드 또는 WIC 자격을 확인하면 최대 6명이 1인 $3에 주간 입장할 수 있습니다.",
    "finePrint": "WIC 이용자는 유효한 신분증이 필요하며 결제는 EBT가 아닌 수단으로 해야 합니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "식물원 입구에서 가족이 이용 자격 카드를 제시하고 입장하는 모습을 표현한 서비스용 이미지",
    "cardHook": "SNAP EBT·WIC 가구는 최대 6명이 댈러스 아보리텀을 1인 $3에 관람해요.",
    "overview": "SNAP EBT 카드 또는 WIC 자격을 확인하면 최대 6명이 1인 $3에 주간 입장할 수 있습니다.",
    "description": [
      "해당 가구는 최대 여섯 명까지 주간 일반 입장을 1인 $3에 구매할 수 있습니다. SNAP EBT 카드를 보여주거나 WIC 자격과 유효한 신분증을 확인하며, 결제는 EBT가 아닌 수단으로 현장에서 진행해요."
    ],
    "highlights": [
      "최대 6명까지 주간 입장 1인 $3",
      "SNAP EBT 카드 또는 WIC 자격 확인",
      "사전 등록 없이 현장에서 비 EBT 수단으로 결제"
    ],
    "practicalTips": [
      "사전 등록 없이 현장에서 혜택 입장권을 구매할 수 있어요."
    ],
    "titleBase": "댈러스 아보리텀 Museums for All",
    "titleBenefit": "1인 $3",
    "eligibility": {
      "mode": "conditional",
      "segments": [
        "snap",
        "wic",
        "ebt"
      ],
      "detail": "SNAP EBT 카드 소지자와 유효한 신분증을 제시하는 WIC 이용자에게 적용됩니다."
    },
    "booking": {
      "mode": "not-required",
      "detail": "사전 등록 없이 현장에서 비 EBT 결제수단으로 구매합니다."
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Dallas Arboretum and Botanical Garden",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://dallasarboretum.org/museums-for-all",
        "official": true
      }
    ]
  },
  {
    "id": "dcta-reduced-fares",
    "kind": "benefit",
    "city": "Denton",
    "venue": "DCTA services in Denton",
    "address": "Multiple transit locations, Denton, TX",
    "lat": 33.2148,
    "lng": -97.1331,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "discount",
    "category": "transit",
    "attributes": [],
    "actionUrl": "https://www.dcta.net/fares-passes/reduced-fares-discount-programs/Reduced-Fares",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "41af3f8a293622894f4718bd67c46ed4bceba96aa1d47c5a25dea7296d119165",
      "assetHash": "e2a99d13d5b01c1156285e626ab70f27ff5af5beb22098750c317e36fb7d6541",
      "brief": {
        "venueArchetype": "an unbranded North Texas bus transfer platform with a modern low-floor bus",
        "primaryActivity": "eligible riders using reduced-fare credentials before purchasing or validating transit fares",
        "heroElement": "a senior rider and teenage student holding plain eligibility cards near the open bus door",
        "audience": "a small mix of transit riders including a senior and student",
        "time": "weekday morning",
        "season": "year-round",
        "composition": "street-level diagonal view along the bus doorway with the two riders prominent in the foreground",
        "lighting": "clean overcast daylight with subtle reflections on transit glass",
        "palette": "steel blue, silver, marigold, denim, and concrete gray",
        "avoid": [
          "transit agency logos",
          "readable passes",
          "fare amounts",
          "crowded commuter crush"
        ]
      }
    },
    "image": "assets/listings/dcta-reduced-fares-41af3f8a2936.webp",
    "title": "DCTA Reduced Fares — 고령자·장애인·Medicare·학생 할인",
    "dateLabel": "상시 · 서비스별 할인 운임",
    "cost": "자격 대상 할인 운임",
    "summary": "65세 이상, 장애인, Medicare 이용자와 6–18세 학생은 자격 배지를 발급받아 DCTA 할인 운임을 이용할 수 있습니다.",
    "finePrint": "자격 배지는 승차권이 아니므로 서비스별 할인 운임 또는 패스를 별도로 구매해야 합니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "고령자와 학생 승객이 대중교통 승차 전 할인 자격을 확인하는 모습을 표현한 서비스용 이미지",
    "cardHook": "고령자·장애인·Medicare 이용자와 6–18세 학생은 DCTA 할인 운임을 신청할 수 있어요.",
    "overview": "65세 이상, 장애인, Medicare 이용자와 6–18세 학생은 자격 배지를 발급받아 DCTA 할인 운임을 이용할 수 있습니다.",
    "description": [
      "자격 대상자는 Reduced Fare Eligible 배지를 먼저 발급받아야 합니다. 배지는 승차권이 아니므로 이용할 서비스에 맞는 할인 운임이나 패스를 별도로 구매하며, 금액은 서비스별로 달라요."
    ],
    "highlights": [
      "65세 이상·장애인·Medicare·6–18세 학생 대상",
      "Reduced Fare Eligible 배지 사전 발급 필요",
      "배지와 별도로 서비스별 할인 운임·패스 구매"
    ],
    "practicalTips": [],
    "titleBase": "DCTA Reduced Fares",
    "titleBenefit": "고령자·장애인·Medicare·학생 할인",
    "eligibility": {
      "mode": "conditional",
      "segments": [
        "senior",
        "student",
        "other"
      ],
      "detail": "65세 이상, 장애인, Medicare 이용자, 학생 ID가 있는 6–18세 학생에게 적용됩니다."
    },
    "booking": {
      "mode": "required",
      "detail": "할인 운임을 이용하려면 Reduced Fare Eligible 배지를 발급받아야 합니다."
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Denton County Transportation Authority",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://dcta.net/fares-passes/reduced-fares-discount-programs/Reduced-Fares",
        "official": true
      }
    ]
  },
  {
    "id": "fort-worth-botanic-garden-access-to-nature",
    "kind": "benefit",
    "city": "Fort Worth",
    "venue": "Fort Worth Botanic Garden",
    "address": "3220 Botanic Garden Blvd, Fort Worth, TX 76107",
    "lat": 32.7404164,
    "lng": -97.3638283,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "free",
    "category": "access",
    "attributes": [],
    "actionUrl": "https://fwbg.org/plan-a-visit/accessibility/access-to-nature/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "8a93104bfdcd950ef574c397f7590939e7ae0b7779d79376e71b32681d598f99",
      "assetHash": "9792aced02fe9397820f9a4d9a9e516ef23201b72698f72985ac4afe68f068bc",
      "brief": {
        "venueArchetype": "an expansive public botanical garden with layered native plantings and a gently curving path",
        "primaryActivity": "an eligible household enjoying general garden admission together",
        "heroElement": "two adults and young children discovering a cluster of colorful plants beside the path",
        "audience": "one household with children under fifteen",
        "time": "late morning",
        "season": "early autumn transition",
        "composition": "wide ground-level garden scene with the family small but clear along an S-curving path",
        "lighting": "soft sun filtered through mature trees with natural leaf shadows",
        "palette": "fern green, lavender, rust, warm stone, and pale sky",
        "avoid": [
          "admission desk",
          "eligibility cards",
          "exact garden landmarks",
          "formal portrait posing"
        ]
      }
    },
    "image": "assets/listings/fort-worth-botanic-garden-access-to-nature-8a93104bfdcd.webp",
    "title": "포트워스 보태닉 가든 Access to Nature — 가족 일반 입장 무료",
    "dateLabel": "상시 · 공식 운영시간 적용",
    "cost": "성인 2명과 15세 미만 자녀 무료",
    "summary": "SNAP·WIC·Medicaid 가구는 성인 2명과 같은 가구의 15세 미만 자녀가 일반 입장을 무료로 이용할 수 있습니다.",
    "finePrint": "현장에서 해당 자격 증빙과 사진이 있는 신분증을 준비하세요.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "가족이 식물원의 푸른 산책로와 정원을 함께 둘러보는 모습을 표현한 서비스용 이미지",
    "cardHook": "SNAP·WIC·Medicaid 가구는 성인 2명과 15세 미만 자녀가 일반 입장 무료예요.",
    "overview": "SNAP·WIC·Medicaid 가구는 성인 2명과 같은 가구의 15세 미만 자녀가 일반 입장을 무료로 이용할 수 있습니다.",
    "description": [
      "Access to Nature 혜택은 같은 가구의 성인 두 명과 15세 미만 자녀에게 무료 일반 입장을 제공합니다. 현장에서 SNAP, WIC 또는 Medicaid 자격 증빙과 사진이 있는 신분증을 확인해요."
    ],
    "highlights": [
      "성인 2명과 같은 가구의 15세 미만 자녀 무료",
      "SNAP·WIC·Medicaid 가구 대상",
      "현장에서 자격 증빙과 사진 신분증 확인"
    ],
    "practicalTips": [
      "자격 증빙과 사진이 있는 신분증을 현장에 가져가세요."
    ],
    "titleBase": "포트워스 보태닉 가든 Access to Nature",
    "titleBenefit": "가족 일반 입장 무료",
    "eligibility": {
      "mode": "conditional",
      "segments": [
        "snap",
        "wic",
        "medicaid",
        "ebt"
      ],
      "detail": "SNAP, WIC 또는 Medicaid 자격 증빙이 있는 가구에 적용됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Fort Worth Botanic Garden",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://fwbg.org/plan-a-visit/accessibility/access-to-nature",
        "official": true
      }
    ]
  },
  {
    "id": "frisco-oktoberfest-2026",
    "kind": "event",
    "city": "Frisco",
    "venue": "The Star",
    "address": "9 Cowboys Way, Frisco, TX 75034",
    "lat": 33.110649,
    "lng": -96.828384,
    "start": "2026-10-03T11:00:00-05:00",
    "end": "2026-10-03T21:00:00-05:00",
    "costType": "free",
    "category": "festival",
    "attributes": [],
    "actionUrl": "https://www.visitfrisco.com/events/frisco-oktoberfest/6398",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "2b6c39ad6aaabab59e78debcde0e5a2f1cd3797fbe51ea0bb949125bcbc7ccdb",
      "assetHash": "763b0d21196288f4c6060b9f18360fec9e1ffef4fe5fc438b8ecfb27f4cb8cc9",
      "brief": {
        "venueArchetype": "a spacious North Texas outdoor festival plaza decorated with generic Bavarian-inspired colors",
        "primaryActivity": "visitors enjoying live music, traditional games, and German food culture",
        "heroElement": "friends cheering a playful stein-holding contest while a small brass band performs nearby",
        "audience": "a lively mixed-age daytime festival crowd",
        "time": "Saturday afternoon",
        "season": "early October",
        "composition": "energetic eye-level wide shot with the game in front and musicians layered in the middle distance",
        "lighting": "clear autumn daylight with crisp colorful shadows",
        "palette": "cobalt, white, mustard, chestnut, evergreen, and warm red",
        "avoid": [
          "sports-team branding",
          "exact plaza architecture",
          "readable banners",
          "intoxication"
        ]
      }
    },
    "image": "assets/listings/frisco-oktoberfest-2026-2b6c39ad6aaa.webp",
    "title": "제7회 Frisco Oktoberfest",
    "dateLabel": "10월 3일 토요일 · 오전 11시–오후 9시",
    "cost": "입장 무료",
    "summary": "The Star에서 라이브 음악, 전통 게임과 독일식 음식 문화를 즐기는 하루 축제가 열립니다.",
    "finePrint": "입장은 무료이며 음식과 음료는 별도 구매입니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "야외 옥토버페스트에서 방문객이 전통 게임과 라이브 음악을 즐기는 모습을 표현한 서비스용 이미지",
    "cardHook": "라이브 음악과 전통 게임을 즐기는 하루 축제에 무료로 입장해요.",
    "overview": "The Star에서 라이브 음악, 전통 게임과 독일식 음식 문화를 즐기는 하루 축제가 열립니다.",
    "description": [
      "10월 3일 토요일 오전 11시부터 오후 9시까지 The Star에서 열립니다. 라이브 음악, 전통 게임과 독일식 음식 문화를 둘러보는 행사이며 축제 입장에는 비용이 들지 않아요."
    ],
    "highlights": [
      "10월 3일 오전 11시–오후 9시",
      "라이브 음악과 전통 게임",
      "축제 입장은 무료 · 음식과 음료는 별도"
    ],
    "practicalTips": [],
    "titleBase": "제7회 Frisco Oktoberfest",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "일반 방문객에게 공개됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Visit Frisco",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://visitfrisco.com/events/frisco-oktoberfest/6398",
        "official": true
      }
    ]
  },
  {
    "id": "grapefest-free-thursday-2026",
    "kind": "event",
    "city": "Grapevine",
    "venue": "Historic Main Street Grapevine",
    "address": "Historic Main Street District, Grapevine, TX 76051",
    "lat": 32.934767,
    "lng": -97.078864,
    "start": "2026-09-17T11:00:00-05:00",
    "end": "2026-09-17T22:00:00-05:00",
    "costType": "free",
    "category": "festival",
    "attributes": [],
    "actionUrl": "https://www.grapevinetexasusa.com/grapefest/general-information/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "3d2f9b446b1f63c90a0c1b690062ae12cbe3cd5d20b08a3927bf6f2910930cd1",
      "assetHash": "aae9dd0057e1ab2797307c15c050876a1117d62a50b3f67c6f5b8fb42e748ff1",
      "brief": {
        "venueArchetype": "a welcoming historic-style downtown festival street with grape-harvest decorations and vendor tents",
        "primaryActivity": "general visitors exploring the festival district during complimentary Thursday admission",
        "heroElement": "a group of friends walking beneath clusters of purple grape-inspired decorations toward market booths",
        "audience": "an all-ages daytime and early-evening festival crowd",
        "time": "Thursday late afternoon",
        "season": "mid-September",
        "composition": "long street-level perspective with visitors entering from the foreground and booths receding centrally",
        "lighting": "warm late-afternoon sun with soft shade under tents",
        "palette": "grape purple, sage, cream canvas, copper, and dusty rose",
        "avoid": [
          "exact Main Street storefronts",
          "readable festival signage",
          "prominent alcohol consumption",
          "carnival close-up"
        ]
      }
    },
    "image": "assets/listings/grapefest-free-thursday-2026-3d2f9b446b1f.webp",
    "title": "제40회 GrapeFest 목요일 — 하루 종일 입장 무료",
    "dateLabel": "9월 17일 목요일 · 오전 11시–오후 10시",
    "cost": "목요일 일반 입장 무료",
    "summary": "GrapeFest 첫날인 목요일에는 히스토릭 메인 스트리트 축제 구역에 하루 종일 무료로 입장할 수 있습니다.",
    "finePrint": "와인 체험, 카니발 게임, 음식과 일부 부가 프로그램은 별도 요금이 있을 수 있습니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "포도 장식이 있는 거리 축제에서 방문객이 부스와 공연을 즐기는 모습을 표현한 서비스용 이미지",
    "cardHook": "GrapeFest 첫날 목요일은 오전 11시부터 밤 10시까지 입장 무료예요.",
    "overview": "GrapeFest 첫날인 목요일에는 히스토릭 메인 스트리트 축제 구역에 하루 종일 무료로 입장할 수 있습니다.",
    "description": [
      "9월 17일 목요일에는 히스토릭 메인 스트리트 축제 구역에 하루 종일 무료로 입장할 수 있습니다. 무료 혜택은 목요일 축제 입장에 적용되고, 와인 체험이나 카니발 게임 등은 별도일 수 있어요."
    ],
    "highlights": [
      "9월 17일 목요일 하루 종일 입장 무료",
      "오전 11시–오후 10시",
      "히스토릭 메인 스트리트 축제 구역"
    ],
    "practicalTips": [],
    "titleBase": "제40회 GrapeFest 목요일",
    "titleBenefit": "하루 종일 입장 무료",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "9월 17일 목요일 일반 입장객에게 적용됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Grapevine Convention & Visitors Bureau",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://grapevinetexasusa.com/grapefest/general-information",
        "official": true
      }
    ]
  },
  {
    "id": "heard-museum-museums-for-all-5",
    "kind": "benefit",
    "city": "McKinney",
    "venue": "Heard Natural Science Museum & Wildlife Sanctuary",
    "address": "1 Nature Pl, McKinney, TX 75069",
    "lat": 33.158434,
    "lng": -96.615336,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "discount",
    "category": "culture",
    "attributes": [],
    "actionUrl": "https://www.heardmuseum.org/rules-faqs/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "4d96ff3e58a4d3fd5aa6ae2f233102ed34f6b6d9c4094694d0e28f19a6eee275",
      "assetHash": "8c400daa8e0ff51ad6a5c4ce32e18966449ddef8e20aa6920b4e6835d9957623",
      "brief": {
        "venueArchetype": "a rustic-modern natural-science museum welcome desk with subtle wildlife and trail cues",
        "primaryActivity": "an EBT cardholder purchasing reduced admission at the front desk for a small group",
        "heroElement": "a hand presenting a plain benefits card while a child looks toward a nearby nature display",
        "audience": "one adult cardholder with two accompanying visitors",
        "time": "midday",
        "season": "late summer",
        "composition": "close three-quarter desk view with the card exchange sharp and the nature exhibit softly behind",
        "lighting": "warm interior daylight with a cool glimpse of an outdoor trail",
        "palette": "cedar brown, moss green, sandstone, muted turquoise, and cream",
        "avoid": [
          "readable card details",
          "exact museum exhibits",
          "online purchase screens",
          "full outdoor family scene"
        ]
      }
    },
    "image": "assets/listings/heard-museum-museums-for-all-5-4d96ff3e58a4.webp",
    "title": "허드 뮤지엄 Museums for All — 최대 4명 1인 $5",
    "dateLabel": "상시 · 현장 매표",
    "cost": "최대 4명 1인 $5",
    "summary": "EBT 카드를 현장 데스크에 제시하면 최대 4명이 1인 $5에 입장할 수 있습니다.",
    "finePrint": "온라인 구매에는 적용되지 않으며 현장에서 EBT 카드를 제시해야 합니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "자연과학박물관 안내 데스크에서 방문객이 EBT 카드를 제시하는 모습을 표현한 서비스용 이미지",
    "cardHook": "EBT 카드를 현장에 제시하면 최대 4명이 허드 뮤지엄을 1인 $5에 관람해요.",
    "overview": "EBT 카드를 현장 데스크에 제시하면 최대 4명이 1인 $5에 입장할 수 있습니다.",
    "description": [
      "정규 운영시간에 현장 프런트 데스크에서 유효한 EBT 카드를 제시하면 카드 소지자와 동반자 최대 네 명에게 1인 $5 입장이 적용됩니다. 이 혜택은 온라인 구매에는 적용되지 않아요."
    ],
    "highlights": [
      "최대 4명까지 1인 $5",
      "현장 프런트 데스크에서 EBT 카드 제시",
      "온라인 구매에는 적용되지 않음"
    ],
    "practicalTips": [
      "혜택 입장권은 현장 프런트 데스크에서 구매하세요."
    ],
    "titleBase": "허드 뮤지엄 Museums for All",
    "titleBenefit": "최대 4명 1인 $5",
    "eligibility": {
      "mode": "conditional",
      "segments": [
        "ebt"
      ],
      "detail": "유효한 EBT 카드를 현장에서 제시하는 카드 소지자와 동반자에게 적용됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Heard Natural Science Museum & Wildlife Sanctuary",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://heardmuseum.org/rules-faqs",
        "official": true
      }
    ]
  },
  {
    "id": "irving-tribute-to-summer-2026",
    "kind": "benefit",
    "city": "Irving",
    "venue": "The Plaza at Toyota Music Factory",
    "address": "340 W Las Colinas Blvd, Irving, TX 75039",
    "lat": 32.874747,
    "lng": -96.944641,
    "start": "2026-08-08T21:00:00-05:00",
    "end": "2026-08-29T23:59:00-05:00",
    "costType": "free",
    "category": "concert",
    "attributes": [],
    "actionUrl": "https://www.irvingtexas.com/event/a-tribute-to-summer%253A-free-concert-series/40380/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "9825a22b02a790788d27c34004fde51212906940457f7cad53089029f35a496b",
      "assetHash": "c2184cee37b7fe105e989fdeab79303a7e709c1ff21576ea84a7821f906b6925",
      "brief": {
        "venueArchetype": "a modern open-air entertainment plaza with a modest concert stage and broad gathering space",
        "primaryActivity": "the public enjoying a free Saturday-night tribute-band performance",
        "heroElement": "a silhouetted guitarist and vocalist performing under stage lights for a relaxed standing crowd",
        "audience": "an adult and family outdoor concert audience",
        "time": "Saturday at 9 PM",
        "season": "August",
        "composition": "wide crowd-level night view framed by raised hands at the edges and the band centered in depth",
        "lighting": "deep blue night with magenta and amber stage beams",
        "palette": "midnight blue, magenta, amber, black, and electric cyan",
        "avoid": [
          "recognizable performers",
          "band logos",
          "exact venue architecture",
          "daytime picnic atmosphere"
        ]
      }
    },
    "image": "assets/listings/irving-tribute-to-summer-2026-9825a22b02a7.webp",
    "title": "A Tribute to Summer 콘서트 시리즈 — 토요일 밤 무료 공연",
    "dateLabel": "8월 29일까지 매주 토요일 · 오후 9시 시작",
    "cost": "입장 무료",
    "summary": "8월 토요일 밤마다 Toyota Music Factory 야외 플라자에서 트리뷰트 밴드 공연을 무료로 즐길 수 있습니다.",
    "finePrint": "야외 공연이며 각 공연은 오후 9시에 시작합니다. 당일 변경 사항은 공식 페이지에서 확인하세요.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "여름밤 야외 광장에서 관객이 트리뷰트 밴드 공연을 즐기는 모습을 표현한 서비스용 이미지",
    "cardHook": "8월 토요일 밤 9시, 야외 플라자에서 트리뷰트 밴드 공연을 무료로 들어요.",
    "overview": "8월 토요일 밤마다 Toyota Music Factory 야외 플라자에서 트리뷰트 밴드 공연을 무료로 즐길 수 있습니다.",
    "description": [
      "8월 29일까지 매주 토요일 오후 9시에 Toyota Music Factory 야외 플라자에서 공연이 시작됩니다. 트리뷰트 밴드의 라이브 음악을 별도 입장료 없이 즐길 수 있어요."
    ],
    "highlights": [
      "8월 29일까지 토요일마다 진행",
      "각 공연 오후 9시 시작",
      "야외 플라자 입장 무료"
    ],
    "practicalTips": [],
    "titleBase": "A Tribute to Summer 콘서트 시리즈",
    "titleBenefit": "토요일 밤 무료 공연",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "일반 관객에게 공개됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Irving Convention & Visitors Bureau",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://irvingtexas.com/event/a-tribute-to-summer%253A-free-concert-series/40380",
        "official": true
      }
    ]
  },
  {
    "id": "juice-box-the-colony-free-toppings-2026",
    "kind": "benefit",
    "city": "The Colony",
    "venue": "Juice Box The Colony",
    "address": "5701 State Hwy 121 Ste 130, The Colony, TX 75056",
    "lat": 33.078759,
    "lng": -96.862697,
    "start": "2026-08-07T00:00:00-05:00",
    "end": "2026-12-31T23:59:00-06:00",
    "costType": "discount",
    "category": "food",
    "attributes": [],
    "actionUrl": "https://www.visitthecolonytx.com/DocumentCenter/View/9000/Juice_Box_digital-2026?bidId=",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "9d8bc1e26d1619e8b0bd1d1c4b165c884ebf0a79df99ccf422c416f558aa8474",
      "assetHash": "2fff84885e998ccd8708dd0d110afdbaf42d4ddb8be6d4faa188528a6d0f176e",
      "brief": {
        "venueArchetype": "a bright contemporary boba-tea counter with colorful ingredient containers and no branding",
        "primaryActivity": "a dine-in customer presenting a printed coupon while purchasing boba tea with two toppings",
        "heroElement": "a clear unbranded tea cup with two contrasting toppings being handed across the counter beside a folded coupon",
        "audience": "one customer and one drink-shop employee",
        "time": "afternoon",
        "season": "late summer",
        "composition": "tight counter-height product-and-hands view with the cup centered and the coupon secondary",
        "lighting": "bright softbox-like shop lighting with clean translucent highlights",
        "palette": "taro lilac, mango yellow, milk tea tan, black tapioca, and mint",
        "avoid": [
          "readable coupon code",
          "shop logos",
          "more than two featured toppings",
          "sealed delivery order"
        ]
      }
    },
    "image": "assets/listings/juice-box-the-colony-free-toppings-2026-9d8bc1e26d16.webp",
    "title": "주스 박스 더 콜로니 — 보바 토핑 최대 2개 무료",
    "dateLabel": "2026년 12월 31일까지",
    "cost": "보바티 구매 시 토핑 최대 2개 무료",
    "summary": "인쇄 쿠폰과 코드 THECOLONY를 제시하면 보바티 구매 시 토핑을 최대 2개 무료로 받을 수 있습니다.",
    "finePrint": "더 콜로니점 매장 식사 1회 한정, 방문·테이블당 쿠폰 1장만 가능하며 다른 혜택과 중복할 수 없습니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "버블티 매장에서 인쇄 쿠폰을 제시하고 토핑을 추가한 음료를 받는 모습을 표현한 서비스용 이미지",
    "cardHook": "보바티를 사면 인쇄 쿠폰과 코드로 토핑을 최대 2개 무료로 받아요.",
    "overview": "인쇄 쿠폰과 코드 THECOLONY를 제시하면 보바티 구매 시 토핑을 최대 2개 무료로 받을 수 있습니다.",
    "description": [
      "더 콜로니점에서 보바티를 구매하고 인쇄 쿠폰과 코드 THECOLONY를 함께 제시하면 토핑을 최대 두 개 무료로 받을 수 있습니다. 매장 식사 1회에 한하며 방문·테이블당 쿠폰 한 장이 적용돼요."
    ],
    "highlights": [
      "보바티 구매 시 토핑 최대 2개 무료",
      "인쇄 쿠폰과 코드 THECOLONY 필요",
      "더 콜로니점 매장 식사 · 방문·테이블당 1장"
    ],
    "practicalTips": [
      "쿠폰을 인쇄하고 코드 THECOLONY를 함께 제시하세요."
    ],
    "titleBase": "주스 박스 더 콜로니",
    "titleBenefit": "보바 토핑 최대 2개 무료",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "인쇄 쿠폰과 코드 THECOLONY를 제시하는 매장 식사 고객에게 적용됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Visit The Colony",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://visitthecolonytx.com/DocumentCenter/View/9000/Juice_Box_digital-2026?bidId=",
        "official": true
      }
    ]
  },
  {
    "id": "knapp-heritage-park-free-admission",
    "kind": "benefit",
    "city": "Arlington",
    "venue": "Knapp Heritage Park",
    "address": "201 W Front St, Arlington, TX 76011",
    "lat": 32.73806,
    "lng": -97.10856,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "free",
    "category": "culture",
    "attributes": [],
    "actionUrl": "https://www.arlingtontx.gov/News-Articles/2026/March/Knapp-Heritage-Park-Preserving-the-Past-in-Downtown-Arlington",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "16a908c4f6ca64f04984205f492fa4242e28f733b5cd9cd2b1f6b40f7831e610",
      "assetHash": "2066b8461ee4c16831cad011f20724b41eb8ad7cf25ed7b7b48b09df80bc27c7",
      "brief": {
        "venueArchetype": "a compact downtown heritage park with generic preserved wooden and brick buildings",
        "primaryActivity": "visitors freely exploring local history and historic structures",
        "heroElement": "an adult and child walking between two restored period buildings while studying architectural details",
        "audience": "a few quiet weekend visitors",
        "time": "Saturday late morning",
        "season": "early autumn",
        "composition": "slightly low wide courtyard view using the historic rooflines to frame the walking pair",
        "lighting": "soft clear daylight with textured shadows on aged wood and brick",
        "palette": "weathered red, faded white, prairie grass, oak green, and denim blue",
        "avoid": [
          "exact historic buildings",
          "costumed reenactors",
          "readable plaques",
          "large museum interior"
        ]
      }
    },
    "image": "assets/listings/knapp-heritage-park-free-admission-16a908c4f6ca.webp",
    "title": "냅 헤리티지 파크 — 입장 무료",
    "dateLabel": "토 오전 10시–오후 2시 · 일 오후 1시–4시",
    "cost": "무료 · 기부 가능",
    "summary": "다운타운 알링턴의 역사 건물과 지역 유산을 무료로 둘러볼 수 있습니다.",
    "finePrint": "운영시간은 토요일과 일요일에 다릅니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "방문객이 도심 속 역사 공원의 오래된 건물들을 둘러보는 모습을 표현한 서비스용 이미지",
    "cardHook": "다운타운 알링턴의 역사 건물과 지역 유산을 입장료 없이 둘러봐요.",
    "overview": "다운타운 알링턴의 역사 건물과 지역 유산을 무료로 둘러볼 수 있습니다.",
    "description": [
      "토요일 오전 10시–오후 2시와 일요일 오후 1–4시에 개방됩니다. 공원 안 역사 건물과 알링턴 지역 유산을 무료로 둘러볼 수 있고 기부도 받습니다."
    ],
    "highlights": [
      "역사 건물과 지역 유산 관람 무료",
      "토요일 오전 10시–오후 2시",
      "일요일 오후 1–4시 · 기부 가능"
    ],
    "practicalTips": [],
    "titleBase": "냅 헤리티지 파크",
    "titleBenefit": "입장 무료",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "일반 방문객에게 개방됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "City of Arlington",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://arlingtontx.gov/News-Articles/2026/March/Knapp-Heritage-Park-Preserving-the-Past-in-Downtown-Arlington",
        "official": true
      }
    ]
  },
  {
    "id": "lewisville-fiesta-charra-2026",
    "kind": "event",
    "city": "Lewisville",
    "venue": "Bill Weaver Arena",
    "address": "101 Parkway Dr, Lewisville, TX 75057",
    "lat": 33.04867,
    "lng": -96.99321,
    "start": "2026-09-06T16:00:00-05:00",
    "end": "2026-09-06T22:30:00-05:00",
    "costType": "free",
    "category": "festival",
    "attributes": [],
    "actionUrl": "https://www.cityoflewisville.com/city-hall/city-departments/community-engagement/special-events-festivals/fiesta-charra",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "fde7175ca9a5096770f357fd9ebe7bd3aebd4254067f6492264034cdc841f751",
      "assetHash": "d38487cd8d72e6454bdf4fb65db27c146742e1a91fe11af9a69aaceb74ae8abf",
      "brief": {
        "venueArchetype": "an open-air community arena prepared for a traditional charreada-inspired cultural celebration",
        "primaryActivity": "visitors watching horsemanship alongside live music and cultural performances",
        "heroElement": "a skilled rider guiding a horse through a controlled arena turn as musicians wait at the rail",
        "audience": "a broad family crowd seated around the arena",
        "time": "Sunday near sunset",
        "season": "Labor Day weekend",
        "composition": "dynamic wide arena view with the horse and rider crossing the foreground and spectators wrapping behind",
        "lighting": "golden sunset dust with warm practical lights beginning to glow",
        "palette": "saddle brown, marigold, deep red, turquoise, and sunset peach",
        "avoid": [
          "unsafe horse handling",
          "exact arena architecture",
          "readable sponsor signs",
          "rodeo branding"
        ]
      }
    },
    "image": "assets/listings/lewisville-fiesta-charra-2026-fde7175ca9a5.webp",
    "title": "Fiesta Charra 2026",
    "dateLabel": "9월 6일 일요일 · 오후 4시–10시 30분",
    "cost": "입장 무료",
    "summary": "전통 차레아다에서 영감을 받은 승마 공연, 라이브 음악, 레슬링과 문화 공연을 무료로 즐길 수 있습니다.",
    "finePrint": "현장 잔디 주차는 $10이며 Lewisville City Hall의 무료 주차·셔틀을 이용할 수 있습니다. 반려동물은 셔틀에 탈 수 없습니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "야외 아레나에서 기마 공연과 문화 행사를 즐기는 관객의 모습을 표현한 서비스용 이미지",
    "cardHook": "승마 공연·라이브 음악·레슬링과 문화 공연을 무료입장으로 즐겨요.",
    "overview": "전통 차레아다에서 영감을 받은 승마 공연, 라이브 음악, 레슬링과 문화 공연을 무료로 즐길 수 있습니다.",
    "description": [
      "9월 6일 일요일 오후 4시부터 10시 30분까지 Bill Weaver Arena에서 열립니다. 전통 차레아다에서 영감을 받은 승마 공연에 라이브 음악, 레슬링과 문화 공연이 이어져요."
    ],
    "highlights": [
      "9월 6일 오후 4시–10시 30분",
      "승마 공연·라이브 음악·레슬링·문화 공연",
      "행사 입장 무료"
    ],
    "practicalTips": [
      "Lewisville City Hall에 무료 주차하고 셔틀을 이용할 수 있어요."
    ],
    "titleBase": "Fiesta Charra 2026",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "가족을 포함한 일반 방문객에게 공개됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "City of Lewisville",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://cityoflewisville.com/city-hall/city-departments/community-engagement/special-events-festivals/fiesta-charra",
        "official": true
      }
    ]
  },
  {
    "id": "lewisville-library-card-free-texas",
    "kind": "benefit",
    "city": "Lewisville",
    "venue": "Lewisville Public Library",
    "address": "1197 W Main St, Lewisville, TX 75067",
    "lat": 33.042921,
    "lng": -97.02331,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "free",
    "category": "library",
    "attributes": [],
    "actionUrl": "https://library.cityoflewisville.com/about/library-accounts/library-card-privileges",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "fbd71aefb45d0ec2a1f832bb197ae1d5d18ce5438d7fd22c20f13e99a17d4c51",
      "assetHash": "43c0c0b4a1f72b7a06613629bdd5a2a592fb474cef24dd7647bfbf4becff27cd",
      "brief": {
        "venueArchetype": "a spacious public library registration table beside tall shelves and a bright reading area",
        "primaryActivity": "a Texas resident applying in person for a free library card",
        "heroElement": "an adult reviewing a simple application folder while a staff member places a plain card beside it",
        "audience": "one adult applicant and one library employee",
        "time": "early evening",
        "season": "year-round",
        "composition": "overhead-oblique tabletop view with hands, application folder, and card arranged against deep shelf perspective",
        "lighting": "cool window light balanced by warm reading lamps",
        "palette": "navy, warm brass, cherry wood, paper white, and muted green",
        "avoid": [
          "readable application text",
          "personal data",
          "same close counter handoff composition",
          "branded library card"
        ]
      }
    },
    "image": "assets/listings/lewisville-library-card-free-texas-fbd71aefb45d.webp",
    "title": "루이스빌 공공도서관 카드 — 텍사스 거주자 무료",
    "dateLabel": "상시 · 도서관 카드 신청",
    "cost": "텍사스 거주자 무료",
    "summary": "텍사스 거주자는 신분과 현재 주소를 확인해 루이스빌 공공도서관 카드를 무료로 발급받을 수 있습니다.",
    "finePrint": "Lewisville 외 텍사스 거주자는 직접 방문해 신청해야 하며 미성년자는 연령별 보호자 조건이 적용됩니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "텍사스 주민이 도서관에서 신청서를 확인하고 새 카드를 받는 모습을 표현한 서비스용 이미지",
    "cardHook": "텍사스 거주자는 신분과 주소 확인 후 루이스빌 도서관 카드를 무료로 받아요.",
    "overview": "텍사스 거주자는 신분과 현재 주소를 확인해 루이스빌 공공도서관 카드를 무료로 발급받을 수 있습니다.",
    "description": [
      "정부 발행 사진 신분증과 필요시 현재 주소 증빙으로 신청합니다. Lewisville 성인은 온라인 신청이 가능하고, 다른 텍사스 지역 거주자는 도서관을 직접 방문해야 해요."
    ],
    "highlights": [
      "텍사스 거주자 도서관 카드 무료",
      "정부 발행 사진 신분증과 필요시 주소 증빙",
      "Lewisville 외 텍사스 거주자는 직접 방문 신청"
    ],
    "practicalTips": [],
    "titleBase": "루이스빌 공공도서관 카드",
    "titleBenefit": "텍사스 거주자 무료",
    "eligibility": {
      "mode": "conditional",
      "segments": [
        "resident",
        "library-card"
      ],
      "detail": "텍사스 거주자는 신분과 현재 주소를 확인해 무료 카드를 신청할 수 있습니다."
    },
    "booking": {
      "mode": "required",
      "detail": "Lewisville 성인은 온라인 신청이 가능하며 그 외 텍사스 거주자는 직접 방문해야 합니다."
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "City of Lewisville",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://library.cityoflewisville.com/about/library-accounts/library-card-privileges",
        "official": true
      }
    ]
  },
  {
    "id": "mckinney-trick-or-treat-trail-2026",
    "kind": "event",
    "city": "McKinney",
    "venue": "Towne Lake Park",
    "address": "1405 Wilson Creek Pkwy, McKinney, TX 75069",
    "lat": 33.18342,
    "lng": -96.63493,
    "start": "2026-10-17T15:00:00-05:00",
    "end": "2026-10-17T18:00:00-05:00",
    "costType": "free",
    "category": "family",
    "attributes": [],
    "actionUrl": "https://www.mckinneytexas.org/calendar.aspx?EID=35353",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "389dd8faff7d202e62672f90aec201bdc19e56be2323c96c42010b8e280ffc87",
      "assetHash": "77785e8d21e76af1076327b85176d31d6965294a6a92cb7ddfcf5cb8bb021321",
      "brief": {
        "venueArchetype": "a winding lakeside community-park trail lined with small daytime trick-or-treat activity stations",
        "primaryActivity": "families following the long trail for trick-or-treating and children's activities",
        "heroElement": "costumed children carrying small treat buckets toward the next volunteer station along the path",
        "audience": "families with young children and community volunteers",
        "time": "Saturday mid-afternoon",
        "season": "mid-October",
        "composition": "long-lens landscape view compressing the winding trail, colorful stations, and walking families",
        "lighting": "gentle autumn afternoon sun filtered through trees",
        "palette": "pumpkin orange, lake blue, leaf gold, plum, and soft green",
        "avoid": [
          "nighttime carnival",
          "frightening horror",
          "exact park landmarks",
          "crowded single plaza"
        ]
      }
    },
    "image": "assets/listings/mckinney-trick-or-treat-trail-2026-389dd8faff7d.webp",
    "title": "맥키니 Trick or Treat Trail",
    "dateLabel": "10월 17일 토요일 · 오후 3시–6시",
    "cost": "무료",
    "summary": "Towne Lake Park의 1.2마일 트레일을 따라 가족과 함께 트릭 오어 트릿과 어린이 활동을 즐길 수 있습니다.",
    "finePrint": "날씨에 따른 변경 여부는 공식 시 캘린더에서 확인하세요.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "가을 공원 산책로에서 가족들이 트릭 오어 트리트와 어린이 활동을 즐기는 모습을 표현한 서비스용 이미지",
    "cardHook": "1.2마일 공원 트레일을 따라 트릭 오어 트릿과 어린이 활동을 무료로 즐겨요.",
    "overview": "Towne Lake Park의 1.2마일 트레일을 따라 가족과 함께 트릭 오어 트릿과 어린이 활동을 즐길 수 있습니다.",
    "description": [
      "10월 17일 토요일 오후 3–6시에 Towne Lake Park에서 진행됩니다. 가족이 1.2마일 트레일을 따라 이동하며 트릭 오어 트릿과 어린이 활동에 참여할 수 있어요."
    ],
    "highlights": [
      "10월 17일 오후 3–6시",
      "Towne Lake Park의 1.2마일 트레일",
      "트릭 오어 트릿과 어린이 활동 무료"
    ],
    "practicalTips": [],
    "titleBase": "맥키니 Trick or Treat Trail",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "가족을 포함한 일반 방문객에게 공개됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "City of McKinney",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://mckinneytexas.org/calendar.aspx?EID=35353",
        "official": true
      }
    ]
  },
  {
    "id": "modern-fort-worth-free-fridays",
    "kind": "benefit",
    "city": "Fort Worth",
    "venue": "Modern Art Museum of Fort Worth",
    "address": "3200 Darnell St, Fort Worth, TX 76107",
    "lat": 32.749286,
    "lng": -97.363069,
    "start": "2026-08-07T00:00:00-05:00",
    "end": null,
    "costType": "free",
    "category": "culture",
    "attributes": [],
    "actionUrl": "https://www.themodern.org/visit/hours-and-admission",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "9f6ffa4fd8f2e7733d8e184dac8f33748b5468f4674db72fc03c74774af4cfef",
      "assetHash": "37b51571d8f54fdfb358b948688156e80d1818e606a97964a92c3daa9c2b4afb",
      "brief": {
        "venueArchetype": "a serene large-scale contemporary-art gallery with broad walls and a sculptural bench",
        "primaryActivity": "Friday visitors exploring permanent and special exhibitions with free admission",
        "heroElement": "two visitors seated on a long bench considering a generic abstract installation across the gallery",
        "audience": "a sparse group of independent adult museum visitors",
        "time": "Friday late afternoon",
        "season": "year-round",
        "composition": "architectural ultra-wide view with the seated pair low in frame and large negative gallery space above",
        "lighting": "cool diffuse ceiling light with a narrow wash of late-afternoon sun",
        "palette": "concrete gray, white, black, rust orange, and muted ultramarine",
        "avoid": [
          "recognizable artworks",
          "exact museum architecture",
          "hands-on art tables",
          "crowded family program"
        ]
      }
    },
    "image": "assets/listings/modern-fort-worth-free-fridays-9f6ffa4fd8f2.webp",
    "title": "포트워스 모던 아트 뮤지엄 — 금요일 입장 무료",
    "dateLabel": "매주 금요일 · 오전 10시–오후 8시",
    "cost": "금요일 무료",
    "summary": "금요일에는 상설전과 특별전 등을 무료 입장으로 관람할 수 있습니다.",
    "finePrint": "무료 입장은 금요일에 적용됩니다. 방문 전 공식 페이지에서 당일 운영 정보를 확인하세요.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "현대미술관의 넓은 전시실에서 방문객이 작품을 감상하는 모습을 표현한 서비스용 이미지",
    "cardHook": "금요일 오전 10시부터 오후 8시까지 상설전과 특별전을 무료로 관람해요.",
    "overview": "금요일에는 상설전과 특별전 등을 무료 입장으로 관람할 수 있습니다.",
    "description": [
      "매주 금요일에는 포트워스 모던 아트 뮤지엄 입장료가 무료입니다. 오전 10시부터 오후 8시까지 상설전과 특별전을 포함한 금요일 관람에 혜택이 적용돼요."
    ],
    "highlights": [
      "매주 금요일 입장 무료",
      "금요일 오전 10시–오후 8시",
      "상설전과 특별전 포함"
    ],
    "practicalTips": [],
    "titleBase": "포트워스 모던 아트 뮤지엄",
    "titleBenefit": "금요일 입장 무료",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "금요일 방문객에게 적용됩니다."
    },
    "booking": {
      "mode": "not-stated"
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Modern Art Museum of Fort Worth",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://themodern.org/visit/hours-and-admission",
        "official": true
      }
    ]
  },
  {
    "id": "plano-dine-discover-pass-2026",
    "kind": "benefit",
    "city": "Plano",
    "venue": "Participating Plano locations",
    "address": "Multiple locations, Plano, TX",
    "lat": 33.0198,
    "lng": -96.6989,
    "start": "2026-03-02T00:00:00-06:00",
    "end": "2026-09-30T23:59:00-05:00",
    "costType": "free",
    "category": "local-deals",
    "attributes": [],
    "actionUrl": "https://www.visitplano.com/dineanddiscover/",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "1a6767a473da5b9020f71e4c245d474ac28099ce87e6ebfc81112ddd69b2e823",
      "assetHash": "0882a640fa8fe4d1b8d2cf9a4ffb70735839c0400a9cd92f567110ef20f0d182",
      "brief": {
        "venueArchetype": "a casual independent restaurant table in a lively North Texas mixed-use district",
        "primaryActivity": "a visitor using a free mobile discovery pass to check in before enjoying a participating location",
        "heroElement": "a hand holding a phone with an abstract unreadable pass screen above a table set for a local meal",
        "audience": "two adult friends exploring participating restaurants and attractions",
        "time": "weekend brunch",
        "season": "late summer",
        "composition": "top-down three-quarter table scene with the phone central and the friends partially visible at opposite edges",
        "lighting": "bright window light with crisp food and tabletop texture",
        "palette": "tomato red, avocado green, warm ceramic, sky blue, and charcoal",
        "avoid": [
          "readable app text",
          "restaurant logos",
          "specific reward claims",
          "phone-only product shot"
        ]
      }
    },
    "image": "assets/listings/plano-dine-discover-pass-2026-1a6767a473da.webp",
    "title": "2026 플래이노 Dine & Discover Pass — 모바일 패스 무료",
    "dateLabel": "2026년 9월 30일까지",
    "cost": "패스 무료 · 참여 혜택은 업체별 상이",
    "summary": "무료 모바일 패스로 플래이노의 참여 식당과 명소를 둘러보고 업체별 할인·혜택과 체크인 보상을 확인할 수 있습니다.",
    "finePrint": "혜택 내용은 참여 업체마다 다르며 패스 등록이 필요합니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "플레이노의 식당에서 방문객이 휴대전화의 모바일 패스로 체크인하는 모습을 표현한 서비스용 이미지",
    "cardHook": "무료 모바일 패스로 플래이노 참여 식당·명소의 할인과 체크인 보상을 찾아봐요.",
    "overview": "무료 모바일 패스로 플래이노의 참여 식당과 명소를 둘러보고 업체별 할인·혜택과 체크인 보상을 확인할 수 있습니다.",
    "description": [
      "2026년 3월 2일부터 9월 30일까지 참여 업체별 할인과 혜택, 체크인 보상을 확인할 수 있습니다. 무료 등록 후 문자나 이메일로 패스를 받으며 별도 앱 설치는 필요하지 않아요."
    ],
    "highlights": [
      "2026년 9월 30일까지 무료 모바일 패스",
      "참여 식당·명소별 할인·혜택과 체크인 보상",
      "문자 또는 이메일 수령 · 별도 앱 불필요"
    ],
    "practicalTips": [
      "패스는 문자나 이메일로 받아 별도 앱 없이 사용할 수 있어요."
    ],
    "titleBase": "2026 플래이노 Dine & Discover Pass",
    "titleBenefit": "모바일 패스 무료",
    "eligibility": {
      "mode": "general",
      "segments": [
        "all"
      ],
      "detail": "무료 모바일 패스에 등록한 이용자에게 적용됩니다."
    },
    "booking": {
      "mode": "required",
      "detail": "문자 또는 이메일로 패스를 받기 위한 무료 등록이 필요합니다."
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Visit Plano",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://visitplano.com/dineanddiscover",
        "official": true
      }
    ]
  },
  {
    "id": "strikz-kids-bowl-free-2026",
    "kind": "benefit",
    "city": "Frisco",
    "venue": "Strikz Entertainment",
    "address": "8789 Lebanon Rd, Frisco, TX 75034",
    "lat": 33.120619,
    "lng": -96.808062,
    "start": "2026-03-23T00:00:00-05:00",
    "end": "2026-09-11T23:59:00-05:00",
    "costType": "free",
    "category": "family",
    "attributes": [],
    "actionUrl": "https://strikz.com/kids-bowl-free",
    "visual": {
      "type": "generated-editorial",
      "promptVersion": 1,
      "key": "0256aa1480bbf2b2b48ebbe5d5d7b52f3f478ca40a1980d68ce14d17b28d2901",
      "assetHash": "57dbe2367df7098a2dbe2f18cf0c3a8177cd47230d23985ee821e699f4ea91d7",
      "brief": {
        "venueArchetype": "a clean unbranded family bowling center with classic lanes and ball returns",
        "primaryActivity": "registered children enjoying their included daily classic-lane bowling games",
        "heroElement": "a child releasing a bowling ball while two other children cheer from the approach seating",
        "audience": "a small group of children age fifteen and under with a supervising adult",
        "time": "weekday afternoon",
        "season": "summer program season",
        "composition": "low lane-level view following the rolling ball toward pins with cheering children sharp at the side",
        "lighting": "bright family-center ceiling light with subtle lane reflections",
        "palette": "royal blue, cherry red, maple wood, white, and lime accents",
        "avoid": [
          "venue logos",
          "shoe rental claims",
          "cosmic bowling darkness",
          "professional competition"
        ]
      }
    },
    "image": "assets/listings/strikz-kids-bowl-free-2026-0256aa1480bb.webp",
    "title": "스트릭즈 Kids Bowl Free — 어린이 매일 2게임 무료",
    "dateLabel": "2026년 9월 11일까지 · 요일별 이용시간 적용",
    "cost": "등록 어린이 2게임 무료 · 신발 별도",
    "summary": "15세 이하 등록 어린이는 공개된 이용시간에 클래식 레인 볼링 2게임을 매일 무료로 이용할 수 있습니다.",
    "finePrint": "신발 대여는 포함되지 않으며 클래식 레인만 가능하고 이용 가능 여부에 따릅니다. Rose Room은 제외됩니다.",
    "actionLabel": "공식 정보 확인",
    "imageAlt": "어린이들이 실내 클래식 볼링 레인에서 함께 게임을 즐기는 모습을 표현한 서비스용 이미지",
    "cardHook": "등록한 15세 이하 어린이는 매일 클래식 레인 볼링 2게임을 무료로 이용해요.",
    "overview": "15세 이하 등록 어린이는 공개된 이용시간에 클래식 레인 볼링 2게임을 매일 무료로 이용할 수 있습니다.",
    "description": [
      "3월 23일부터 9월 11일까지 공개된 이용시간에 등록 어린이 한 명당 하루 두 게임이 무료입니다. 클래식 레인만 대상이며 이용 가능 여부에 따르고 신발 대여는 포함되지 않아요."
    ],
    "highlights": [
      "15세 이하 등록 어린이 하루 2게임 무료",
      "3월 23일–9월 11일 공개 이용시간",
      "클래식 레인만 가능 · 신발 대여 별도"
    ],
    "practicalTips": [
      "이용 전에 Kids Bowl Free 등록을 완료해야 해요."
    ],
    "titleBase": "스트릭즈 Kids Bowl Free",
    "titleBenefit": "어린이 매일 2게임 무료",
    "eligibility": {
      "mode": "conditional",
      "segments": [
        "children"
      ],
      "detail": "15세 이하 어린이를 등록해야 합니다."
    },
    "booking": {
      "mode": "required",
      "detail": "Kids Bowl Free 등록이 필요합니다."
    },
    "verifiedAt": "2026-08-07T21:32:52.000Z",
    "sources": [
      {
        "name": "Strikz Entertainment",
        "note": "공식 원문에서 핵심 날짜·장소·가격·이용 조건 확인",
        "url": "https://strikz.com/kids-bowl-free",
        "official": true
      }
    ]
  }
];

export const RESEARCH_NOTE = {
  "verifiedAt": "2026-08-07",
  "methodology": "주최 측·도시·기관의 공식 안내에서 날짜·가격·이용 대상·장소를 먼저 확인하고, 별도 검색으로 취소·변경·가격 충돌을 다시 확인했어요. 공식 확인이 불가능한 경우에는 등록된 독립 출처 세 곳의 핵심 정보가 모두 일치해야 해요.",
  "disclaimer": "운영 시간, 정원, 날씨, 매진 여부는 바뀔 수 있습니다. 출발 전 카드의 공식 링크에서 마지막 확인을 권장합니다."
};
