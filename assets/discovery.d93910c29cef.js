...
export function isRecentlyVerified(listing, now = new Date(), days = 7) {
  const verifiedAt = new Date(listing.verifiedAt);
  const elapsed = now - verifiedAt;
  return Number.isFinite(verifiedAt.getTime()) && elapsed >= 0 && elapsed <= days * 86_400_000;
}
...
export function lowestPrice(listing, searchText) {
  ...
  const amounts = [...searchText.matchAll(/\$\s*(\d+(?:\.\d+)?)/g)].map((match) =>
    Number(match[1]),
  );
...
export function matchesIntent(listing, intent, context) {
  ...
  if (intent === "kids") {
    return (
      listing.category === "family" ||
      /아이와|가족|전 연령|키즈|kids?|family|all ages/.test(searchText)
    );
  }
  if (intent === "date") {
    return /라이브 음악|재즈|예술|갤러리|영화|성인|콘서트|축제|마켓|live music|jazz|art|gallery|film|adult|concert|festival|market/.test(
      searchText,
    );
  }
  if (intent === "outdoor") {
    return (
      listing.category === "outdoors" ||
      /야외|공원|정원|트레일|outdoors?|park|garden|trail/.test(searchText)
    );
  }
  if (intent === "indoor") {
    return /실내|박물관|갤러리|도서관|전시|indoors?|museum|gallery|library|exhibit/.test(
      searchText,
    );
  }
  if (intent === "food") {
    return /음식|푸드|먹|푸드 트럭|food|eat|restaurant|happy hour/.test(searchText);
  }
  if (intent === "museums") {
    return /박물관|미술관|갤러리|전시|museum|gallery|exhibit/.test(searchText);
  }
  if (intent === "festivals") {
    return listing.category === "festival" || /축제|페스티벌|festival|fest/.test(searchText);
  }
  return true;
}
